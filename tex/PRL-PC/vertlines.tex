...
simplify the problem, we will extract observations of the altitude of 
...
\begin{equation}
\myvec{B_{v}} = \left(\begin{array}{cc}\cos(90-\theta) ~ ~ -\sin(90-\theta)\\\sin(90-\theta) ~  ~  ~ ~ ~ \cos(90-\theta)\end{array}\right) \myvec{B} \label{uprightbaseline}
\end{equation}

{\parindent 0mm
where $\theta$ is the orientation of the $\myvec{B}$ with the horizontal.
Henceforth, we may disregard the $x$-coordinates and deal
solely in the $y,z$ plane. 
In our ideal geometrical model, the bottom of a single paragraph is positioned 
at $\myvec{P}$ with lines occurring at even spacings of distance $\myvec{Q}$.  
Therefore the $n$th line from the bottom of the paragraph will appear at:
}
...
However, since it is common for documents to also contain lines of text which are not part of an evenly spaced block of text, and for extraneous elements to enter the data, the $n$th line observed in the image may lose correspondence with the $n$th line in the paragraph model.
...
For example, in \reffig{poorfitting}, data point (or line number) 7 is an undesirable outlier
which has shifted all successive points one position along the horizontal $n$-axis,
causing a poor fitting.
\begin{figure}[t]
\centering
\begin{center}
	\subfigure[Poor fitting using line position due to the presence of an outlier.]{
		\epsfig{figure=images/fitting/fit-simple-outlier-with-got.eps,width=65mm}
		\label{poorfitting}
	}
	\subfigure[Using line spacing, the outlier does not affect the remaining points.]{
...
With this approach, any odd lines will appear as isolated outliers with an incorrect line spacing, but the error will not propagate through the remaining $X_n$. % points.
By substituing \refeqn{spacingsvweqn} into the definition of line spacing \refeqn{linespacingsdefneqn}, the curve of $Y$ in terms of $X$ may be written as: 
%in two parts:
}
...
Due to the complexity of \refeqn{fittingeqn} and \refeqn{nfromxeqn}, many false minima exist, and one of these may be converged upon during optimisation.
...
\reffig{goodfitting} shows that the outlier from \reffig{poorfitting} has now affected two of the data points, but does not affect the rest of the data.
...
After reversing the transformation made earlier in (\ref{uprightbaseline}) to bring the baseline upright, this point will correspond to the location of the vertical vanishing point in the original image.
...
suitable for documents with jagged edges. We keep the  presentation of  our
