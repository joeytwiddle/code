...
public class CyberPet extends Applet implements ActionListener {
  ...
  public void init () {
    // Set colours of framee) { setBackground (Color.white);
    setForeground (new
		   Color ((float) 12 / 255, (float) 104 / 255,
			  (float) 100 / 255));
  ...
  // My shortcut add button methodett("Octo the cyber pet!");
    void ab (Button b, String s) { b = new Button (s);
    b.setActionCommand (s);
    b.addActionListener (this);
    add (b);
  } public void start () {	// Applet only
    // Resume animationbutton methodett("Octo the cyber pet!");
      octo.at.resume ();
    ...
    // Suspend threadsationif light is onOcto the cyber pet!");
      octo.at.suspend ();
    octo.bt.suspend ();
    octo.ct.suspend ();
  } public void actionPerformed (ActionEvent e) {
    // Call appropriate method for the button presseder pet!");
  ...
  } public void paint (Graphics g) { Color fg = getForeground ();
...
/*	// Debugging data
    ...
    // Stop the current Octo "+(octo.tiredness)+", Illness "+(octo.illness)+".",0,200);*/
      octo.stop ();
    ...
    // Present dialog box and get filenameness)+", Illness "+(octo.illness)+".",0,200);*/
      FileDialog fd =
      new FileDialog (new Frame (), "Load pet", FileDialog.LOAD);
      fd.show ();
    String s = fd.getFile ();
  ...
  void save () {
    // Present dialog box and get filenamerame)+", Illness "+(octo.illness)+".",0,200);*/
      FileDialog fd =
      new FileDialog (new Frame (), "Save pet", FileDialog.SAVE);
      fd.show ();
    String s = fd.getFile ();
    // If not cancelled then
    if (s != null) {
...
/*	// A new Octo has most buttons enabled, and some disabled
    ...
    // Creates and starts new threadsological and coms threadsed
    ...
    // Stops threadsff, biology should be suspendedoms threadsed at.stop ();
    bt.stop ();
    ct.stop ();
  } public void paint (Graphics g, Color fg, Color bg) { int l;
    double ang;
      g.setColor (fg);
    // Mess
    for (l = 1; l <= messes; l++) {
    ...
    // Heads threadsff, biology should be suspendedoms threadsed
      g.setColor (bg);
    ...
    if (tired () || !lighton || octowinks == "Left") {	// tired or asleep or winking eye
      g.drawLine (xposition - 7 - 2, yposition - 15, xposition - 7 + 2,
		  yposition - 15);
    } else {			// normal eye
    ...
    for (l = 0; l <= 7; l++) {
      ang =
	Math.PI / 3 * (l - 3.5) / 3.5 * (0.8 +
					 0.3 * Math.sin (Math.PI * frame /
							 20));
      g.drawLine (xposition, yposition,
		  xposition + (int) (30 * Math.sin (ang)),
		  yposition + (int) (30 * Math.cos (ang)));
    }
    // Mouth (expresses some of current state)
    boolean mouthdrawn = false;
    if (ill ()) {		// ill mouth
      g.drawLine (xposition - 3, yposition - 4, xposition + 3,
		  yposition - 10);
      g.drawLine (xposition - 3, yposition - 10, xposition + 3,
		  yposition - 4);
      mouthdrawn = true;
    }
    ...
    if (!mouthdrawn && messy ()) {	// messy mouth
      g.drawLine (xposition - 7, yposition - 6, xposition + 7,
		  yposition - 10);
      mouthdrawn = true;
    }
  ...
  // Methods changing Octo's statetet state)endedoms threadsed
    public void feed () { food++;
    if (food > 4)
      food = 4;
    ct.showmessagefor ("Yum!", 2);
  }
  // Playing methodsg Octo's statetet state)endedoms threadsed
    public void startplay () {
    // User presses play to start a little gamendedoms threadsed
      playing = true;
...
/*	// Enable/disable relevant buttons
    ...
    // User presses left/right to conclude gamendedoms threadsed
      playing = false;
/*	// Enable/disable relevant buttons
	cpf.playbut.setEnabled(true);
	cpf.leftbut.setEnabled(false);
  ...
  public void clean () { mess--;
  ...
  } public void medicine () { illness = -1;
    ct.showmessagefor ("I feel tip-top now!", 2);
  } public void light () { tiredness = 0;
    ...
    // If it's being turned on, resume his biologys biologyadsed
    else {
      bt.resume ();
  ...
  public void leavehome () { stillathome = false;
...
// Handy methods// Expected number of mess globules for Octo's mess value.",0,200);*/
    int expectedmesses () { return (int) (maxmesses * mess / 4);
  }
  // Returns true with probability 1/pf mess globules for Octo's mess value.",0,200);*/
    boolean rnd (int p) {
    int q = r.nextInt () % p;
    if (q < 0)
        q = -q;
  ...
  // And also a display methodon the screens globules for Octo's mess value.",0,200);*/
    void paint (Graphics g) { g.fillOval (x - 1, y - 1, 3, 3);
}}
// General virtual pet thread, extended for use lateror Octo's mess value.",0,200);*/
  class mythread extends Thread implements Serializable {
  // Thread has access to frame and to Octoor use lateror Octo's mess value.",0,200);*/
    CyberPet cpf;
  octopus octo;
  int sleeptime;
  // Constructor stores info
    ...
    // Keep loopingstores infoame and to Octoor use lateror Octo's mess value.",0,200);*/
      while (true) {
...
void moment () {}}
// Octo's animation extendedbefore going around againor Octo's mess value.",0,200);*/
  class animthread extends mythread {
animthread (CyberPet temp1, octopus temp2, int temp3) {
    super (temp1, temp2, temp3);
  } void moment () {
    // If light is on, Octo movesefore going around againor Octo's mess value.",0,200);*/
      if (octo.lighton) {
      octo.xposition = octo.xposition + octo.direction;
      if (octo.rnd (50) || octo.xposition < 50
	  || octo.xposition > cpf.getSize ().width - 50)
...
class biothread extends mythread {
  biothread (CyberPet temp1, octopus temp2, int temp3) {
    ...
    // Octo digests some foodnimation is fasteround againor Octo's mess value.",0,200);*/
    ...
    // Octo gets more tiredugh mess, add another mess globule to the screenue.",0,200);*/
...
class comsthread extends mythread {
  ...
  int showfor = 0;
    ...
    s = temp1;
    showfor = temp2;
  } void moment () { boolean done = false;
    ...
    // Otherwise, if Octo has any ailments, he lets the user know!me, reset health200);*/
