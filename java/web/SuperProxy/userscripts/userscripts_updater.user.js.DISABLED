...
function updateScripts(evt)
{
	...
	//div result
	div = createElement('div', {id:"divUpdater", style:"background-color:#FFFFAA; position:absolute; top:3px; right:3px; min-width:250px; min-height:50px; max-width:50%; padding:5px; font-size:small; text-align:left; z-index:"+(zindex++)+";"});
...
function options()
{
	...
	//div options
	divoptions = createElement('div', {id:"divOptions", style:"text-align:left; background-color:#FFFF77; position:absolute; top:3px; right:3px; min-width:250px; min-height:50px; padding:5px; font-size:small; color:#222222; z-index:"+(zindex++)+";"});	
	...
	if(titles){
		divoptions.appendChild(createElement('span', '', '', "<br><br>Registered scripts:<br>"));
		
		//scroll box
		divScripts = createElement('div',{id:"divRegisteredScripts", style:"overflow:auto; background-color:#FFFFAA; height:150px; padding:3px; "});
...
function importFromDisk(evt)
{
	...
	if(!state && !confirm('This will try to import scripts from your Firefox profile. Continue?'))
		return;
			...
			switch(state)
			{
				case 'getDirPath': // done in new tab
					if(location.href != 'about:cache')
						throw "e";
					try{
						path = getTag('body')[0].innerHTML.match(/\<tt\>\s+(\D\:\\[^\<]+)/)[1].replace(/\\[^\\]+$/,'').replace(/\\/g,'\/');
					}catch(e){
					}
					if(path)//windows
					{
						conf = path.match(/\D:\/[^\/]+\/[^\/]+\/([^\/]+\/)/)[1];
						rex = new RegExp(conf);
						path = 'file:///' + path.replace(rex,'') + '/gm_scripts/config.xml';
						GM_setValue('importPath', escape(path.replace(/[^\/]+$/,'')));
					}
					else//linux?
					{
						// <tt> /home/username/.mozilla/firefox/vkuuxfit.default/Cache</tt>
						path = getTag('body')[0].innerHTML.match(/\<tt\>\s+(\/home\/[^\<]+)/)[1].replace(/[^\/]+$/,'');
						if(!path)
							throw 'Firefox profile path not found';
						path = 'file:///' + path + '/gm_scripts/config.xml';
				...
				case 'showScriptsFound': //done by caller tab
					...
					getTag('body')[0].appendChild( createElement( 'div', 
						{id:'divImport', style:'text-align:left; background-color:#FFFF77; position:absolute; top:3px; left:3px; min-width:250px; min-height:50px; padding:5px; font-size:small; color:#222222; z-index:'+(zindex++)+';'}, 
						null, 
						'<span id="spanImportSearchCount" style="color:#749EC9; font-weight:bold">Searching...</span><br>'+
...
function importSearch()//search  scripts sequentially
{
	...
	GM_xmlhttpRequest({
		...
		onload: function(resp) {
			...
			var rex = new RegExp('<a[^>]+>'+objImportSearch.name.replace(/([@_'".*+?^${}()|[\]\/\\])/g,'\\$1')+'</a>', 'g');
			...
			if(!links || links.length!=1){
				...
				objImportSearch.scriptSpanContent.innerHTML = 'Search for ';
				objImportSearch.scriptSpanContent.appendChild(createElement('a',{target:'_blank', href:'http://userscripts.org/scripts/search?q='+objImportSearch.name.replace(/\s+/g,'+')}, null, objImportSearch.name));
			}else{
			...
			//search counter...
			getId('spanImportSearchCount').innerHTML = 'Searching... ' + (parseInt(objImportSearch.indexSpan)+1) +'/'+objImportSearch.arraySpans.length;
...
function debug(str, parent, id)
{
	var el=null;
	if(!parent)
		parent = getTag('body')[0];
	if(id){
		if(!(el=getId(id)))
			parent.appendChild(createElement('textarea', {id:id, style:"height:400px; width:100%;"}));
		getId(id).innerHTML += '\n' + str;	
	}else{
		parent.appendChild( createElement('br'));
		parent.appendChild( createElement('textarea', {style:"height:400px; width:100%;"}, null, str));
	}
}
