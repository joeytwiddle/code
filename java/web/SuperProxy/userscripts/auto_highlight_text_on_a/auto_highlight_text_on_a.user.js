...
// The original algorithm was designed to work on only one word.  Now we are
// calling it multiple times.  It is inefficient re-traversing the DOM for each
// word.  It would be better to check all the words inside the node check.
*/
...
if (words) {
	...
	*/
	...
	// But this method might be considered better.  It staggers the work.
	// This allows the browser to render the page, and allows the user a little interaction time,
	// before the browser becomes occupied again (freezes) or the next chunk of work.
	function setupTimers() {

		var wordsLeft = words;
		var searchFn = searchWithinNode;

		function doWord() {
			if (wordsLeft.indexOf(" ")<0) {
				searchFn(window.document.body, wordsLeft.toUpperCase(), wordsLeft.length);
				return;
			}
			var firstWord = wordsLeft.substring(0,wordsLeft.indexOf(" "));
			wordsLeft = wordsLeft.substring(wordsLeft.indexOf(" ")+1);
			if (firstWord.length>0) {
				searchFn(window.document.body, firstWord.toUpperCase(), firstWord.length);
			}
			setTimeout('doWord();',200);
		}

		unsafeWindow.doWord = doWord;

		unsafeWindow.setTimeout('doWord();',200);
