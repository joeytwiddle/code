...
public class VisualJavaGUI {
    ...
    public VisualJavaGUI() {
        ...
        new Thread() { public void run() { addMenuBar(frame); } }.start();
	...
	public void addMenuBar(final JFrame frame) {
            ...
            new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                    ...
                    if (result == JFileChooser.APPROVE_OPTION) {
                    	...
                    	try {
                    		Object obj = new XStream(new DomDriver()).fromXML(new FileReader(file));
                    	...
                    	try {
                    		...
                    		for (int i=0;i<components.length;i++) {
                    			if (components[i] instanceof VariableView) {
                    				...
                    				try {
                    					String dummyResult = new XStream(new DomDriver()).toXML(variable);
                    					variables.add(variable);
                    				} catch (Exception e2) {
                    					Logger.warn("Could not convert to XML "+variable+" because "+e2);
                    				}
                    			}
                    		}
                    		// Object obj = this;
	    ...
	    ActionListener actionListener = new ActionListener() {
	        public void actionPerformed(ActionEvent e) {
	            ...
	            if (e.getSource() instanceof JCheckBoxMenuItem) {
	                VisualJavaOptions.autoExecNoParameterMethods = ((JCheckBoxMenuItem)e.getSource()).isSelected();
	                if (!VisualJavaOptions.autoExecNoParameterMethods) {
	                	// TODO: setSelected(false) and setEnabled(false) on the other option
	                } else {
	    ...
	    actionListener = new ActionListener() {
	        public void actionPerformed(ActionEvent e) {
	            Logger.info("Action on "+e.getActionCommand()+": "+e);
	            if (e.getSource() instanceof JCheckBoxMenuItem) {
	                VisualJavaOptions.autoCloseNoParameterMethods = ((JCheckBoxMenuItem)e.getSource()).isSelected(); 
	            }
	        }
	    };
	    menuItem.addActionListener(actionListener);
...
//		frame.setJMenuBar(menuBar);
	...
	public JMenu buildClasspathMenu() {
        ...
        // No good adding all libs; since they aren't neccessarily on the classpath!  (We get ClassNotFound exceptions when we try to access them.)
        /*
        File[] children = jrelibdir.listFiles();
        for (int i=0;i<children.length;i++) {
        	if (children[i].isFile() && children[i].getName().toLowerCase().endsWith(".jar")) {
        		classPath += classPathSeparator + children[i].getAbsolutePath();
        	}
        }
        */
        // But we do want rt.jar:  (although it may already be there, but it isn't for me under Windows)
        classPath += classPathSeparator + new File(jrelibdir,"rt.jar").getAbsolutePath();
					...
					public void actionPerformed(ActionEvent arg0) {
	                    ...
	                    if (result == JFileChooser.APPROVE_OPTION) {
	                    	...
	                    	for (int i=0;i<files.length;i++) {
	                    		...
	                    		// TODO BUG: the property changes but the classes are not accessible to java :S
	                    		String classpath = System.getProperty("java.class.path");
	                    	    ...
	                    	    JCheckBoxMenuItem menuItem = buildClasspathSelection(file.getAbsolutePath());
	                    	    menu.add(menuItem);
	                    	    // menu.validate();
	                    	    // menu.repaint();
	                    	}
        ...
        try {
            ...
            while (true) {
                ...
                if (line == null)
                    break;
                if (line.length() == 0) {
                    org.neuralyte.Logger.error("VisualJavaGUIStatics.buildClassMenu(): Got class = \"\"!");
                }
            ...
            // }
    			...
    			// classMenu.generateChildren(); // Doesn't tell itself that caching has been done!
    			...
    			/*
         new LoudThread("ensurePopulated") {
         ...
         menu.ensurePopulated();
    			...
    			if (statusBar != null) {
					...
					statusBar.setStatus(classMenu.countTotalContainedClasses+" classes available.");
    			}
    ...
    public void printClassesTo(final PrintWriter writer) {
        ...
        try {
            ...
            org.neuralyte.Logger.error(e); // If we can't create the PipedOutputStream (never happens)
        }
...
//                Thread.currentThread().setPriority(1);
                ...
                org.neuralyte.Logger.log("Will parse classpath: " + classPath);
                int j;
                while (classPath.length() > 0) {
                    ...
                    if (statusBar != null) {
                        statusBar.setStatus("Parsing class names from "  + jarOrDir + " (" + classPath.split(":").length + " remaining on classpath) ...");
                    }
                    String command;
                    printClassListFromResource(writer, jarOrDir);
                }
                ...
                */
                ...
                for (int i=0;i<classpathMenu.getComponentCount();i++) {
                    JCheckBoxMenuItem checkBoxMenuItem = (JCheckBoxMenuItem)classpathMenu.getComponent(i);
                    if (checkBoxMenuItem.isSelected()) {
                        printClassListFromResource(writer, checkBoxMenuItem.getLabel());
                    }
                }
                */
                
                for (int i=0;i<classpathSelections.size();i++) {
                    JCheckBoxMenuItem checkBoxMenuItem = (JCheckBoxMenuItem)classpathSelections.get(i);
                    if (checkBoxMenuItem.isSelected()) {
                    	ClassMenuBuilder.printClassListFromResource(writer, checkBoxMenuItem.getLabel());
                    }
                }
