...
// Temporary state:
 ...
 var config bool bExportHighScoresLists;
 // var config float LadderPointsLostPerDay;
 var config float MinHoursForFullScore; // How long before they get their full score
 var config float MinHoursToRetainFullScore; // Influences how fast they fall down the ladder
 var String Top20[20];
 var float Top20Score[20];
 var float Top20Age[20];
 var String Top20IP[20];
 var int NumPlayerScores;
 var String Top20Combined,Top5Combined;
 var String PlayerScoresCombined;
 var config bool bExportCountries;
 var Actor IpToCountry;
 var int DoneSortListsCount;
 var config String IptcCache[64][2];
// Default values:
defaultproperties {
 bLogging=True
 bDebugLogging=False
 ...
 bExportHighScoresLists=True
 bExportCountries=False
 MinHoursForFullScore=1.0
 MinHoursToRetainFullScore=3.0
}
...
function PostBeginPlay() {
 ...
 CleanupDatabase();
  if (bExportHighScoresLists) {
   // We do it early for old versions of ATBHighScores, and to populate the list for players who join the server really fast.
   SortLists();
   // But we need do it later when the IpToCountry actor is available.
   // And again to pick up the results requested from IpToCountry.  :S
  }
 initialized = true;
}
// ISSUES: It seems IpToCountry doesn't immediately return results already cached in IpData.  O_o
// Perhaps it will do so after a normal end-game?
// Without caching, lookups of many IPs can take a long time, so the flag data may not be available for 10+ seconds.
// We could cache country alongside players in ATB DB, to make them resolve faster.
// We could also store real player IPs, since atm we only store their ISP.
// ISSUES: Lookup of country for every player is overkill, likely to run very
// slow, and flood the IpData table!  It would be better to store index numbers
// for players as we build the high score table, and then only lookup for those
// who made it into the Top20/5.  However, caching as above would also solve
// this.
// STATUS: ATB runs SortLists every few seconds for a while, hoping eventually to get the needed results from IpToCountry.  TODO: If IpToCountry won't answer immediately, then we will need to implement our own IpData cache.  Also/alternatively, we need not request country of players until the final list is made.  (No need querying 100 countries for top 20 list!)  We'll need to collect Top20I[i] I guess, as stated in previous issue
// BUG TODO: ATB seems to be refreshing date_last_played[i] as soon as a player joins a game.  This might be useful for ATB to know to keep a player record active, even if the player doesn't finish the game.  But for highscores it kinda sucks.  Perhaps we can isolate it and switch it just for ATBHS releases.
// BTW noggin did you notice that UnrealScript is Object Oriented, so you can
// use inheritance to extend code, rather than building something monolithic
// with #ifdefs?  STFU noggin.
function SortLists() {
 local Actor A;
 local int i;
 ; if (bDebugLogging) { Log("+AutoTeamBalance+ "$ PrePad(Int(Level.TimeSeconds)," ",4) $" "$ "Doing SortLists()"); };
 // We will use IpToCountry if we can find it.
 if (bExportCountries) {
  if (IpToCountry == None) {
   // It does not have that tag during PostBeginPlay.
   // But if we check on a late enough timer, it does.
   ...
   /*
			if (IpToCountry == None) {
				// Set a random actor so we won't search again.
				IpToCountry = Level;
				// No good: We may need to search again later!
			}
			*/
   ; if (bDebugLogging) { Log("+AutoTeamBalance+ "$ PrePad(Int(Level.TimeSeconds)," ",4) $" "$ "IpToCountry = "$IpToCountry); };
  }
 }
 BringTopPlayersWithin(1.0); // 1 day
 Top5Combined = CombineTop(5);
 BringTopPlayersWithin(4*30.0); // 4 months
 Top20Combined = CombineTop(20);
 ; if (bDebugLogging) { Log("+AutoTeamBalance+ "$ PrePad(Int(Level.TimeSeconds)," ",4) $" "$ "Top20Combined="$Top20Combined); };
 ; if (bDebugLogging) { Log("+AutoTeamBalance+ "$ PrePad(Int(Level.TimeSeconds)," ",4) $" "$ "Done SortLists()"); };
}
function String CombineTop(int num) {
 local String s;
 local int i;
 for (i=0;i<num;i++) {
  if (i > 0) s = s $ "____DELIM____";
  s = s $ Top20[i];
 }
 return s;
}
function BringTopPlayersWithin(float maxAge) {
 local int i;
 local int j;
 local bool skipped;
 local String tma;
 local float Score;
 local float ageOfRecord_i_inDays;
 for (j=0;j<20;j++) {
  Top20[j] = "";
  Top20Score[j] = 0;
  Top20Age[j] = 0;
 ...
 tma = ThisModeAppender();
 // DONE: If gametype signature enabled, filter records by that, to show table only for current gametype.
 for (i=0;i<MaxPlayerData;i++) {
  // TODO: Add efficiency check - if list is full and they score lower than 19th - skip them immediately.  Or perhaps we should just do reverse search?
  // In fact we know the current number of list entries will be Min(j,20)
  // if (hours_played[i] < 0.8 /*MaxHoursWhenCopyingOldRecord + 0.5*/) {
   // continue;
  // }
  if (tma != "" && !StrEndsWith(nick[i],tma)) {
   continue;
  }
  ageOfRecord_i_inDays = (currentDateDays - DaysFromDateString(date_last_played[i]));
  // DebugLog("["$i$"] "$nick[i]$" "$ ageOfRecord_i_inDays $" "$ date_last_played[i] $" "$ CalcLadderScore(i));
  if (ageOfRecord_i_inDays > maxAge) {
   continue;
  }
  Score = CalcLadderScore(i);
  if (Score <= 0) {
   continue;
  }
  // DONE: In absence of UIDs, we should also merge/reject any records from
  // the same IP (not only same-name records).  Yes occasionally this might
  // be two brothers, but more often it's someone just changing their nick.
  // Check for existing entry with same nick:
  skipped=false;
  for (j=0;j<20;j++) {
   if (StrStartsWith(Top20[j],UnFudge(nick[i])$" ") || Top20IP[j]==ip[i]) {
    // BUG: If we change the score for this entry, he may now be at the wrong position in the list!
    /*
				// We don't want to take it just cos the score is larger.
				// if (Score > Top20Score[j]) {
				// We should keep the most recent record.
				if (ageOfRecord_i_inDays < Top20Age[j]) {
					Top20[j] = UnFudge(nick[i]) @ Score @ GetCountryForRecord(i);
					Top20Score[j] = Score;
					Top20Age[j] = ageOfRecord_i_inDays;
					Top20IP[j] = ip[i];
				}
				skipped=true;
				break;
    ...
    // SOLUTION:
    // if (ageOfRecord_i_inDays < Top20Age[j]) {
    //// On second thought let's be nice and take whatever his highest record is.  This seems reasonable now that CalcLadderScore is auto fading old records, and also auto-fading small fresh records.
    if (Score > Top20Score[j]) {
     // We delete the existing entry, and add the new one (don't set skipped)
     UnshiftFrom(j);
    } else {
     skipped = true;
    }
  ...
  // If not, find where in the list he belongs:
  if (!skipped) {
   for (j=0;j<20;j++) {
    if (Score > Top20Score[j] || Top20[j]=="") {
     ShiftFrom(j);
     Top20[j] = UnFudge(nick[i]) @ Score @ GetCountryForRecord(i);
     Top20Score[j] = Score;
     Top20Age[j] = ageOfRecord_i_inDays;
...
// Generates a ladder score from ATB record i.
// Unfortunately ATB wasn't designed for this, and didn't collect the right stats for it, so here we have a rather strained attempt using only avg_score[i], hours_played[i] and date_last_played[i].
function float CalcLadderScore(int i) {
 local float Score;
 local float daysOld;
 // return avg_score[i];
 Score = avg_score[i];
 daysOld = (currentDateDays - DaysFromDateString(date_last_played[i]));
 // DONE: _D_ We could reduce effect _A_ below, but increase effect _B_ *when* the players hoursPlayed is low.  I.e. this doesn't affect current score, but when absent, short records fall faster than long records.  I.e. one-minute wonders can occupy a high place in the scoreboard for a short time, but records who have really proved themselves stay up there for longer.
 // _A_ Allow new players to appear in the scoreboard, but don't give them their full score until they have played for a while.
 if (hours_played[i] < MinHoursForFullScore) {
  Score = Score * (hours_played[i]/MinHoursForFullScore);
 }
 // For people with dynamic IPs, anything above an hour might seem unfair.
 // But to properly earn a place, skill should be proven, not a one off lucky strike.
 // So I recommend a min of 1.5 hours, perhaps up to 3.0, to really earn the top of the scoreboard.
 // _C_ Reducing score by absence causes the player's score to jump up suddenly when they join a game.
 // This can make the Top20/"Your score" differ because only "Your score:" displays the new value.
 // Score -= LadderPointsLostPerDay * daysOld;
 // _B_ Player retains his score for 24 hours
 if (24.0*daysOld < 24.0) {
  // Do nothing.
 } else {
  // After 1 day, we begin to consider his record as old.
  daysOld -= 1.0; // Start from then
  // We reduce his score so that in 1 year it will become 0 (after 6 months it is halved).
  // _D_ Except we modify the dropoff if his record is small.
  if (hours_played[i] < MinHoursToRetainFullScore) {
   daysOld *= (1 + 4*(1.0-hours_played[i]/MinHoursToRetainFullScore));
   // If he played for 0 hours his record falls five times as quickly.
   // If he played for MinHoursToRetainFullScore/2, his record falls 3 times as quickly.
  }
  Score = Score * (1.0 - daysOld / 365.25);
  // That means his score drops 1/52th each week.
  // For a star player at 900, that's 18 points per week, or 2.467 per day.
 }
 // BUG: daysOld is pretty volatile, it might be easily refreshed without much
 // play.  A player with a high score can turn up, not play out the game, but
 // perhaps trigger refresh of his record.
 // strength 100 = ladder 1000
 return 10.0 * Score;
}
function ShiftFrom(int j) {
 local int i;
 for (i=18;i>=j;i--) {
  Top20[i+1] = Top20[i];
  Top20Age[i+1] = Top20Age[i];
  Top20Score[i+1] = Top20Score[i];
  Top20IP[i+1] = Top20IP[i];
 }
}
function UnshiftFrom(int j) {
 local int i;
 for (i=j;i<=18;i++) {
  Top20[i] = Top20[i+1];
  Top20Age[i] = Top20Age[i+1];
  Top20Score[i] = Top20Score[i+1];
  Top20IP[i] = Top20IP[i+1];
 }
}
function String GetCountryForRecord(int i) {
 local String ipdata;
 ...
 if (IpToCountry != None && IpToCountry != Level) {
  ipdata = IpToCountry.GetItemName(ip[i] $ append);
  ; if (bDebugLogging) { Log("+AutoTeamBalance+ "$ PrePad(Int(Level.TimeSeconds)," ",4) $" "$ "Got IPTC data for "$ip[i]$" >>> "$ ipdata); };
  if (Left(ipdata,1) == "!") {
   return "";
  }
  SplitString(ipdata,":",args);
  return args[4]; // 2-code lowercase country field
 }
 return "";
}
...
event Timer() {
 ...
 if (gameStartDone)
  ...
  if (bExportHighScoresLists) {
   // For Country lookup we need to repeat the query, until IpToCountry return the data.
   // The more lookups we do, the longer it takes to get the results from IpToCountry.  So this needs testing on a big DB.
   if (bExportCountries && DoneSortListsCount<14 && !gameStartDone) {
    DoneSortListsCount++;
    if (DoneSortListsCount%4 == 1) {
     SortLists();
    }
   }
  }
...
function ModifyPlayer(Pawn paw) {
 ...
 if (p!=None && p.PlayerReplicationInfo.Deaths == 0 && Spectator(p)==None) {
   // BUG: These stats are rubbish.  Far preferable would be points *earned* today, so any noob can get there by spamming hard and long; currently 5 great players can dominate today's top5 with a short visit.
   if (bExportHighScoresLists) {
    PlayerScoresCombined = PlayerScoresCombined $" "$ Int(paw.PlayerReplicationInfo.PlayerID%128) $":"$ CalcLadderScore(FindPlayerRecordGuaranteed(p)) $" ";
   }
