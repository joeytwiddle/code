/* WARNING! This file was auto-generated by jpp.  You probably want to be editing ./FastBuildMut.uc.jpp instead. */
...
function FindBoundary() {
 ...
 if (bConfineToBase) {
  foreach AllActors(class'PlayerStart', spawnPoint) {
   if (spawnPoint.TeamNumber>=0 && spawnPoint.TeamNumber<=2) {

    if ((VSize(TeamOrigin[spawnPoint.TeamNumber] - spawnPoint.Location)) > MaxAllowedRadius) {
     MaxAllowedRadius = (VSize(TeamOrigin[spawnPoint.TeamNumber] - spawnPoint.Location));
...
event Timer() {
 ...
 if (!bFastBuildOver) {
  if (SecondsToGo <= 0) {
   ...
   FlashToAllPlayers("FastBuild is over",colorWhite,2,0,False,True);
   if (bMoreFlashing) {
    FlashToAllPlayers("Weapons are LIVE!",colorWhite,3,0,True,False);
    FlashToAllPlayers("Assault the Base!",colorWhite,4,0,True,False);
   }
...
function AddRUGlobally(float amount) {
 ...
 // We are going to calculate teamScale, so that FastBuildRUPerSecond is fair for e.g. 3v2.
 // To be really fair, this should also be true for the rest of RU collection during normal siege once the fastbuild is over.
 ...
 foreach AllActors(class'Actor', a) {
  ...
  // if (sgPRI(a)!=None && Pawn(a)!=None) { // eh :f
   ...
   // TODO CHECK: For really small cases (24v24), If this is <1.0 and data is int and AddRU does not do it probabilistically or fractionally, then we should!
   sgPRI(a).AddRU(amount * teamScale);

   // Although FastBuild wasn't designed for SiegeAnywhere, it can work in *some* of those types.
   // FastBuild is meant to run with classic Siege, where MaxRU is dealt with at game start and then by the cores.
   // E.g. But if SiegeCTF starts with MaxRU 100, we should expand it somehow as players earn it.
   if (sgPRI(a).RU >= sgPRI(a).MaxRU) {
    // We will do it even in normal Siege FastBuild games, if bRUGetsMaxxedOut=false.
