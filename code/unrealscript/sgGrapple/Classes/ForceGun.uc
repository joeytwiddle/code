/* WARNING! This file was auto-generated by jpp.  You probably want to be editing ./ForceGun.uc.jpp instead. */
...
replication {
 ...
 unreliable if (Role==ROLE_Authority)
  GrabMesh; // Making this unrealiable stopped pickup up item from continuing to float back up for a while after being dropped.
 // reliable if (Role==ROLE_Authority)
...
/*simulated*/ function AcquireTarget() {
 ...
 if (Target == Instigator || Target == Self || Target==Level) {
  if (bLogging) { Super.Log(Self$".BeginState() Bad Target = "$Target); };
...
simulated function InitTarget() {
 ...
 if (bLogging) { Super.Log(Self$".InitTarget() Target="$Target$" UpdateCount="$UpdateCount$""); };
 ...
 Target.SetPhysics(PHYS_Rotating); // TODO: ? can we keep it moving but take out the gravity?
 /*
...
/*simulated*/ function ReleaseTarget() {
 if (Target != None) {
  ...
  if (bLogging) { Super.Log("ReleaseTarget() Released "$Target$" with Physics="$Target.Physics); };
  Target = None;
 }
}
simulated function Tick(float DeltaTime) {
 ...
 if (Level.NetMode!=1 && Instigator==None) {
  ...
  if (Owner != None && Pawn(Owner)!=None) {
   if (bLogging) { Super.Log(Self$".Tick() Set Instigator from Owner "$Owner); };
...
simulated function CheckForUpdate(float DeltaTime) {
 ...
 if (Level.NetMode!=1) {
  ...
  if (Target==None) {
   if (LastTarget!=None) {
    if (bLogging) { Super.Log(Self$".Tick() Dropped LastTarget="$LastTarget); };
  ...
  if (Target!=None && Target!=LastTarget) {
   ...
   if (bLogging) { Super.Log(Self$".Tick() - Client got Target update!  ("$LastTarget$") -> Target="$Target); };
   LastTarget = Target;
   ...
   if (UpdateCount > LastUpdate+1) {
    if (bLogging) { Super.Log(Self$".Tick() We have missed "$ (UpdateCount-LastUpdate-1) $" updates!"); };
   }
   LastUpdate = UpdateCount;
  }
 }
}
simulated function GrabMesh(ForceGun gun, Actor dec, Actor LTarget) {
 // if (VSize(dec.Location-LTarget.Location)<5.0 || dec.Instigator == LTarget || dec.Owner == LTarget) {
 if (dec.Owner == LTarget) {
  if (FRand()<0.01) { if (bLogging) { Super.Log(Self$".MoveMeshes() Caught mesh "$dec$" for LTarget "$LTarget); }; }
  // First time
  dec.SetPhysics(PHYS_Rotating);
 ...
 foreach AllActors(class'ForceGun',gun) {
  ...
  if (LTarget != None) {
   ...
   if (gun.Owner.IsA('Bot')) {
    if (bLogging) { Super.Log("Moving meshes for Bot.Target! "$gun.OWNER.getHumanName()$" "$gun.Target); };
   }
   // sgBuilding will have a corresponding mesh actor
   ...
   // if (sgBuilding(LTarget)!=None) {
    // if (sgTargetMesh==None) {
     ...
     foreach AllActors(class'Effects',TargetEffects) {
      meshCount++;
      GrabMesh(gun,TargetEffects,LTarget);
     }
     foreach AllActors(class'MeshActor',TargetMesh) {
      meshCount++;
      GrabMesh(gun,TargetMesh,LTarget);
     }
     foreach AllActors(class'sgMeshFX',sgTargetMesh) {
      meshCount++;
      GrabMesh(gun,sgTargetMesh,LTarget);
     }
     if (sgTargetMesh==None && TargetMesh==None && TargetEffects==None && FRand()<0.01) {
      if (bLogging) { Super.Log(Self$".MoveMeshes() FAILED to find Mesh for "$LTarget$", searched "$meshCount); };
     }
    // }
    // } else {
...
simulated function BrownianRotate(Actor A) {
 ...
 if (sgBuilding(A)==None || Abs(A.RotationRate.Pitch)>0) { if (FRand()<0.5) { A.RotationRate.Pitch-=5; } else { A.RotationRate.Pitch+=5; } if (A.RotationRate.Pitch<MaxRotRate) { A.RotationRate.Pitch=-MaxRotRate; } else if (A.RotationRate.Pitch>MaxRotRate) { A.RotationRate.Pitch=MaxRotRate; }; };
 if (sgBuilding(A)==None || Abs(A.RotationRate.Roll)>0) { if (FRand()<0.5) { A.RotationRate.Roll-=5; } else { A.RotationRate.Roll+=5; } if (A.RotationRate.Roll<MaxRotRate) { A.RotationRate.Roll=-MaxRotRate; } else if (A.RotationRate.Roll>MaxRotRate) { A.RotationRate.Roll=MaxRotRate; }; };
 if (sgBuilding(A)==None || Abs(A.RotationRate.Yaw)>0) { if (FRand()<0.5) { A.RotationRate.Yaw-=5; } else { A.RotationRate.Yaw+=5; } if (A.RotationRate.Yaw<MaxRotRate) { A.RotationRate.Yaw=-MaxRotRate; } else if (A.RotationRate.Yaw>MaxRotRate) { A.RotationRate.Yaw=MaxRotRate; }; };
 ...
 // DebugLog("Doing ChangeTargetVelocity("$DeltaTime$" distance="$ Int(VSize(holdPos-Target.Location)) $" tgt.P="$Target.Physics$" ins.P="$Instigator.Physics$" tgt.V="$Int(VSize(Target.Velocity))$" ins.V="$Int(VSize(Instigator.Velocity)));
  ...
  // if (True) { // Doing it on both causes client error: Target$" moved without proper hashing"
   /*
			...
			*/
   Target.Velocity = Target.Velocity*(1.0-DeltaTime/3) + ( 3.0*480.0 * Normal(holdPos-Target.Location) * PullStrength / Difficulty() )*DeltaTime*FClamp(VSize(holdPos-Target.Location)/10,0,1);
   if (VSize(holdPos-Target.Location)<=10.0) {
    // Dampen Target around centre of field.
    Target.Velocity = Target.Velocity * (0.9 + 0.1*VSize(holdPos-Target.Location)/10.0); // 0.95 - 1.0
   }
   ...
   if (Target.Physics != PHYS_Flying) {
    ...
    if (!Target.Move(Target.Velocity*DeltaTime) && FRand()>0.1) {
     if (bLogging) { BroadcastMessage("Target "$Target.getHumanName()$" doing odd movement "$Int(VSize(LastLoc-Target.Location))); } if (bLogging) { Super.Log("[ClientLog] "$ "Target "$Target.getHumanName()$" doing odd movement "$Int(VSize(LastLoc-Target.Location))); };
     // Target.SetLocation(LastLoc);
