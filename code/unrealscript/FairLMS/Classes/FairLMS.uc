...
event Timer() {
 ...
 foreach AllActors(class'Pawn',p) {
  if ((PlayerPawn(p)!=None || Bot(p)!=None) && Spectator(p)==None) {
   ...
   if (!bTwoPlayersLeft && bGameStarted && (!Level.Game.bGameEnded || Level.Game.bOverTime)) {
...
function GiveInitialInventory(Pawn p) {
 ...
 if (bGiveWeapons && LastManStanding(Level.Game)==None) {
  for (i=0;i<20;i++) {
   if (InitialWeapon[i]!="") {
