...
/* WARNING! This file was auto-generated by jpp.  You probably want to be editing ./FairLMS.uc.jpp instead. */
...
event Timer() {
 ...
 if (bGameStarted && (LastManStanding(Level.Game)!=None || InStr(String(Level.Game.Class),"LastManStanding")>=0) && !bTwoPlayersLeft && aliveCount<=2 && FRand()<0.2) { // Dodgy FRand delay to avoid get overwritten by other messages ("X is OUT").
  if (aliveCount == 2) {
 ...
 if (bGiveWeapons /*&& bIsLMSGame*/) {
  for (i=0;i<20;i++) {
   if (InitialWeapon[i]!="") {
    ...
    if (inv == None) {
     if (bLogging) { Log("[FairLMS] " $ " - WARNING - " $ "GiveInitialInventory() Failed to spawn a "$iType$" for "$p.getHumanName()); };
    } else {
     GiveInventory(p,inv);
    }
   }
...
function Inventory GivePickupType(Pawn p, class<Inventory> t) {
 ...
 if (Inv!=None) {
  if (bLogging) { Log("[FairLMS] " $ "GivePickupType() "$p.getHumanName()$" already has a "$Inv$" - destroying it."); };
  Inv.Destroy();
  Inv = None;
 ...
 if (Inv==None) {
  if (bLogging) { Log("[FairLMS] " $ " - WARNING - " $ "GivePickupType() Failed to spawn a "$t); };
 } else {
  GiveInventory(p,Inv);
  // Post-hacks:
  if (Weapon(Inv)!=None) {
 ...
 for (inv=p.Inventory;inv!=None;inv=inv.Inventory) {
  if (Weapon(inv)!=None) {
   if (Weapon(inv).AmmoType==None) {
    if (bLogging) { Log("[FairLMS] " $ " - WARNING - " $ "GiveRandomPowerup() Can not boost ammo for "$inv); };
   } else {
    if (Weapon(inv).AmmoType.default.AmmoAmount>1) { // Not for deemer
     ...
     if (bLogging) { Log("[FairLMS] " $ " - WARNING - " $ "GiveRandomPowerup() Not boosting ammo for "$inv); };
    }
   }
  }
 }
...
function GiveRandomPowerup(Pawn p, Pawn victim) {
 ...
 for (j=0;j<100;j++) {
  ...
  if (Powerup[i].Type == "FairLMS.AmmoBoost") {
   ...
   type = class<Inventory>( DynamicLoadObject(Powerup[i].Type,class'Class') );
   if (type == None) {
    if (bLogging) { Log("[FairLMS] " $ " - WARNING - " $ "Powerup #"$i$" type \""$Powerup[i].Type$"\" does not exist!"); };
    continue;
   }
   ...
   inv = Spawn(type,p);
   if (inv == None) {
    if (bLogging) { Log("[FairLMS] " $ " - WARNING - " $ "GiveRandomPowerup() Failed to spawn "$type); };
    continue;
   }
   ...
   GiveInventory(p,inv);
   if (bLogging) {
    if (bLogging) { Log("[FairLMS] " $ "Gave "$inv$" to "$p); };
   }
  }
 ...
 if (j==100) {
  if (bLogging) { Log("[FairLMS] " $ " - WARNING - " $ "GiveRandomPowerup() Tried 100 times but could not find a suitable powerup!  Maybe "$p.getHumanName()$" has everything already."); };
