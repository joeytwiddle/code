...
function DoSetProperty(Actor A, String property, String newValue) {
 ...
 // We can set ZoneInfo.ZoneGravity just fine with SetPropertyText(), but we
 // can't set Location because it's a const.  We must use SetVelocity().
 // tmp = A.GetPropertyText(property);
 // if (StrStartsWith(tmp,"(X=") && StrEndsWith(tmp,")")) {
  // Looks like a vector
  //#define StoreVect(STRNAME,FLATNAME) if (property ~= STRNAME) { A.FLATNAME = vector(newValue); }
  //StoreVect("Location",Location)
  //StoreVect("Velocity",Velocity)
  if (property ~= "Location") {
   A.SetLocation( vector(newValue) );
  } /* else if (property ~= "Velocity") {

			A.SetVelocity( vector(newValue) );
