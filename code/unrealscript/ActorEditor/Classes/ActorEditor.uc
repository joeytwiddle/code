...
// ActorEditor
//
//   Lets the admin to change the properties of actors in a running game.
//
//   Lets the admin save the changes so that they are applied every time the
//   map loads.  (This is currently done at PostBeginPlay() but maybe should
//   also be applied at AlwaysKeep() or CheckReplacement().)
//
// Example uses:
//
//   Populating out-of-date ScreenPageSlide Actors with new URLs/servers/text
//   (e.g. on a Portal map).
//
//   Changing ZoneInfo properties (e.g. making w00t a no-kill boost-only map ^^).
//
//   Changing the text on those scrolling ScriptedTextures.

// TODO: The ActorEditor Wizard may also want to: replace a specific actor,
//   replace all actors matching "...", change properties of one/more actors
//   (e.g. by globbing map+actor names).

// TODO CONSIDER: inform players when ActorEditor is making changes to a map.
//   Maybe allow the editor to explain the change in English, and store this
//   message for the map.

// TODO CONSIDER: collect a list of (max 10/100?) favourite properties; use these for auto-searching (finding actors by property value), or display all present favourite properties on "!examine"

// TODO: to make the config easier to edit, make a meta instruction set, and put commands one-per-line, space-delimited (and execute them like normal messages? -- mmm the "SET" command currently requires "LOOK" or "SEARCH" first.)
...
//       maybe taking action during CheckReplacement() or AlwaysKeep() or PreBeginPlay() could fix this.

// Remove this to leave out my specific Screen support (and Screen dependency).
...
var bool bSaveUserChanges; // TODO: config
...
defaultproperties {
  ...
  bOnlyAdmin=True
  bSaveUserChanges=False // Otherwise, only admin can save changes, with !saveall
...
function bool CheckMessage(String line, PlayerPawn Sender) {
 ...
 if (bLetPlayersSetScreens || Sender.bAdmin || (!bOnlyAdmin)) {
  ...
  // Specifically for Screen actors next to teleporters.
  ...
  // TODO: what if a player has already put a message on this screen?  Ideally we would like the add this new message above or below the previous message (with a max limit?).
  if (command ~= "SETTEXT") {
   ...
   if (args[1] == "") {
    ...
    // Since the default screen font is rather small, if none is specified, we set a larger font:
    if (!StrContains(Caps(rebuilt_string),"<FONT")) {
     rebuilt_string = "<FONT size=+2>" $ rebuilt_string $ "</FONT>";
    }
...
// Actually specific for the setup I have on my CTF-ThornsPortal, which is 1 Teleporter, next to 1 ScreenSlidePageServer (which doesn't work online) and 1 ScreenSlidePageWeb (which does work).
// TODO TEST: If one of the Teleporter/ScreenSlidePageServer/ScreenSlidePageWeb is not present, will the function crash out (or will it just log Accessed None, and proceed to set the next actor)?  If so, which do we want to set first in the code?
