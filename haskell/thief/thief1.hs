...
randpos :: Integer -> [Pos]
randpos seed = zip (map toInt (random (toInteger(1),toInteger(width)) seed)) (map toInt (random (toInteger(1),toInteger(height)) seed))
randdir :: Integer -> [Dir]
randdir seed = map inttodir (random (0,4) seed)
...
-- Shows booglies
showbs :: [Pos] -> IO()
showbs bs = forall bs showb (>>) (return())
            where showb b = plot b "><"
