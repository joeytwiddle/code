...
matches s (Atom a) = if (not (null trymatch)) then
    [Yes (Atom a) s trymatch y]
  else
    []
...
matcheswithrule (t:ts) s = if (not (null result)) then
    theone : therest theone
  else
    []
  where poss = splitstr s
        tryposs (x,y) = if (not (null (matches x t))) && (not (null (matcheswithrule ts y))) then
                          Yes t x (msof (head (matches x t))) y
                        else
                          No
        okay No = False
        okay x = True
        result = filter okay (map tryposs poss)
        theone = head result
        therest (Yes tt ss ms rs) = matcheswithrule ts rs
        msof (Yes o p q r) = q
