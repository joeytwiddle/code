...
rule1 = ( Atom "XML" ,
  [
    [ Str "<" , VarExcl "type" "<> ", Str " ", Atom "Params" , Str ">" , Atom "Inside" , Str "</" , Var "type" , Str ">" ],
...
matches s (VarExcl v e) = if (gotane) then [] else [Yes (VarExcl v e) s [] ""]
  where gotane = foldl (||) False (map test e)
        test i = isin s i

matches s (Str x) = if (x==s) then [Yes (Str s) s [] ""] else []
...
matcheswithrule [] s = []
matcheswithrule xs "" = []
matcheswithrule [x] s = separate (matches s x)
