...
class Date {
public:
  int bottomyear=95; // How to decide century for a two-digit year
  String dateseps="/.";
  ...
  void toString() {
    return Sformat("%i %s %i",d,months.num(m),y);
  }
};
...
int whichmonth(String s) {
  for (int i=1;i<=12;i++) {
    if (Sequiv(s,months.num(i)) || Sequiv(Sleft(s,3),Sleft(months.num(i),3))) {
      return i;
    }
  }
  return 0;
