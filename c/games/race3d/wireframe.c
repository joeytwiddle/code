...
void plotlinenoadd(V3d a,V3d b,int c) {
  if (a.mod()<=7.0) {
  PPlinenoadd(&left,PPlefteye,a,b,c);
  PPlinenoadd(&right,PPrighteye,a,b,c*8);
  }
}
...
void main() {
  ...
  for (float thru=0;thru<1.0;thru+=0.005) {
  ...
  do {
    ...
    for (int i=1;i<=particles.len;i++) {
      ...
      // if (V3d::dot(cen,ori.qz)>0) {
        cen=V3d::disorientate(cen,ori);
        if (cen.z>0) {
          int c=7.0-chop(6.0*(cen.z+PPplane)/(float)(7.0),0,6);
          // int c=7.0-chop(6.0*(i/particles.len),0,6);
          // plotsphere(cen,c);
          V3d q=particles.num(mymod(i+ps,particles.len));
          V3d ocen=q-pos+0.0*ori.y;
          ocen=V3d::disorientate(ocen,ori);
          if (ocen.z>0)
            plotlinenoadd(cen,ocen,c);
          V3d r=particles.num(mymod(i+1,particles.len));
          V3d rcen=r-pos+0.0*ori.y;
          rcen=V3d::disorientate(rcen,ori);
          if (rcen.z>0)
            plotlinenoadd(cen,rcen,c);
        }
      }
