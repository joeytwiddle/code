...
public:
  unsigned char **bmp; // For Dos allegro
  unsigned char **spare;
  int width,height;
  unsigned char buffer[MAXJBMPWIDTH];
  JBmp() {
  }
  ...
  void filledcircle(int cx,int cy,float r,int c) {
    int x,h,y,px,py;
    for (x=-r;x<=r;x++) {
      h=(int)sqrt(mysquare(r)-mysquare(x));
      for (y=-h;y<=h;y++) {
  ...
  void filledcircle(V2d cen,float r,int c) {
    filledcircle(cen.x,cen.y,r,c);
  }
  void filledcirclenodarker(int cx,int cy,int r,int c) {
    int x,h,y,px,py;
    for (x=-r;x<=r;x++) {
      h=(int)sqrt(mysquare(r)-mysquare(x));
      for (y=-h;y<=h;y++) {
  ...
  void shadedcircle(int cx,int cy,int r,int inc,int outc) {
    int x,h,y,px,py;
    for (x=-r;x<=r;x++) {
      h=(int)sqrt(mysquare(r)-mysquare(x));
      for (y=-h;y<=h;y++) {
  ...
  void shadedcirclenodarker(int cx,int cy,int r,int inc,int outc) {
    int x,h,y,px,py;
    for (x=-r;x<=r;x++) {
      h=(int)sqrt(mysquare(r)-mysquare(x));
      for (y=-h;y<=h;y++) {
  ...
  void addcircle(int cx,int cy,int r,int c) {
    int x,h,y,px,py;
    for (x=-r;x<=r;x++) {
      h=(int)sqrt(mysquare(r)-mysquare(x));
      for (y=-h;y<=h;y++) {
  ...
  void filledpolar(int (*fn)(float,float),int cx,int cy,int r,float rot,int c) {
    for (int x=-r;x<=r;x++) {
  ...
  void setpixelnc(int x,int y,int c) {
    bmp[y][x]=c;
  }
  void setpixelud(int x,int y,int c) {
    if (x>=0 && x<width && y>=0 && y<height) {
      bmp[y][x]=c;
      #ifdef ALLEGRO
        blitline(y);
  ...
  void setpixelnodarker(int x,int y,int c) {
    if (x>=0 && x<width && y>=0 && y<height)
      if (bmp[y][x]<c)
      bmp[y][x]=c;
  }
  void addtopixel(int x,int y,int c) {
    setpixel(x,y,bmp[y][x]+c);
  }
  int pointat(int x,int y) {
    int rx=mymod(x,width);
    int ry=mymod(y,height);
    return bmp[ry][rx];
  }
  void setrectangle(int l,int t,int w,int h,int c) {
    if (w<0) {
      l=l+w; w=-w;
    }
    if (h<0) {
  ...
  void hliner(int x,int y,int dx,int c) {
    setrectangle(x,y,dx,1,c);
  }
  void hline(int x,int y,int xb,int c) {
    hliner(x,y,xb-x+1,c);
  }
  void vliner(int x,int y,int dy,int c) {
    setrectangle(x,y,1,dy,c);
  }
  void vline(int x,int y,int yb,int c) {
    vliner(x,y,yb-y+1,c);
  }
  ...
  void line(Line2d l,int c) {
    line(l.a,l.b,c);
  }
  void line(Line2d *l,int c) {
    line(l->a,l->b,c);
  }
  void sthickline(V2d a,V2d b,float w,int c) {
    V2d a2b=w/2.0*(b-a).norm();
    V2d p=a2b.perp();
    filltri(a+p,a-p,b+p,c);
    filltri(a-p,b+p,b-p,c);
  }
  void thickline(V2d a,V2d b,float w,int c) {
    V2d a2b=w/2.0*(b-a).norm();
    V2d p=a2b.perp();
    filltri(a-a2b+p,a-a2b-p,b+a2b+p,c);
    filltri(a-a2b-p,b+a2b+p,b+a2b-p,c);
  }
  void thickline(int xa,int ya,int xb,int yb,float w,int c) {
    thickline(V2d(xa,ya),V2d(xb,yb),w,c);
  }
  void thickline(Line2d l,int w,int c) {
    thickline(l.a,l.b,w,c);
  }
  void thicklinev(V2d a,V2d b,float w,int c) {
    V2d p=V2d(w/2.0,0);
    V2d a2b=V2d(0,w/2.0)*mysgn(b.y-a.y);
    filltri(a-a2b+p,a-a2b-p,b+a2b+p,c);
    filltri(a-a2b-p,b+a2b+p,b+a2b-p,c);
  }
  void thicklineh(V2d a,V2d b,float w,int c) {
    V2d p=V2d(0,w/2.0+1);
    V2d a2b=V2d(w/2.0-1,0)*mysgn(b.x-a.x);
    filltri(a-a2b+p,a-a2b-p,b+a2b+p,c);
    filltri(a-a2b-p,b+a2b+p,b+a2b-p,c);
  }
  void rthickline(V2d a,V2d b,float w,int c) {
    V2d a2b=w/2.0*(b-a).norm();
    V2d p=a2b.perp();
    filltri(a+p,a-p,b+p,c);
    filltri(a-p,b+p,b-p,c);
  ...
  void interpolate(int l,int t,int r,int b,int nwcol,int necol,int swcol,int secol) {
    for (int i=0;i<=r-l;i++)
    for (int k=0;k<=b-t;k++) {
  ...
  Map2d<uchar> tomap() { // Both dependent on same map: can't destroy both!
...
//    printf("...\n");
  ...
  void intvline(int x,int ya,int yb,int ca,int cb) {
    float atob;
    bool done=false;
    int s=sgn(yb-ya);
  ...
  void intertri(V2d a,V2d b,V2d c,int ca,int cb,int cc) {
    intertri(a.x,a.y,b.x,b.y,c.x,c.y,ca,cb,cc);
  }
  void intertri(int xa,int ya,int xb,int yb,int xc,int yc,int ca,int cb,int cc) {
    if (xa>xb) {
...
/*  void filltri(int xa,int ya,int xb,int yb,int xc,int yc,int c) {
  ...
  void filltri(V2d a,V2d b,V2d c,int d) {
    filltri(a.x,a.y,b.x,b.y,c.x,c.y,d);
  }
  void fillpoly(Polygon2d p,int c) {
...
void fillquad(int xa,int ya,int xb,int yb,int xc,int yc,int xd,int yd,int c) {
...
void fillquad(V2d a,V2d b,V2d c,V2d d,int z) {
  filltri(a,b,c,z);
  filltri(a,c,d,z);
//  line(a,b,z);
//  line(b,c,z);
...
void fillstar(int x,int y,int r,float dth,int c) {
  V2d pos=V2d(x,y);
  for (int i=0;i<5;i++) {
...
void opencircle(int x,int y,float r,int c) {
  V2d last=V2d(x,y+r);
  int steps=4+r/2;
...
void opencircle(V2d cen,float r,int c) {
  opencircle(cen.x,cen.y,r,c);
}
uchar chopbmp(int x,int y) {
  x=intmod(x,width);
  y=intmod(y,height);
  return bmp[y][x];
}
