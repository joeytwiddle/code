...
public:
  ...
  Line3d *intersectionheight(float y) {
//    printf("Looking for intersection height %f in %s\n",y,toString());
  ...
  void getintersectionheight(float y,Line3d *l,bool *s) {
//    printf("Looking for intersection height %f in %s\n",y,toString());
    List<V3d> es;
    for (int i=1;i<=vs.len;i++) {
      Line3d l=linefrom(i);
  ...
  Line3d *addifintersectionheight(float y,List<Line3d> *ls) {
    List<V3d> es;
    for (int i=1;i<=vs.len;i++) {
      Line3d l=linefrom(i);
      if (l.a.y>l.b.y)
