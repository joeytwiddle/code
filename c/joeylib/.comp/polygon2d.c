...
// Starts of class Polygon2d
  ...
  // Variable declared in .h file
   ...
   Polygon2d::Polygon2d(List<V2d> v) {
    regionised=NULL;
    smallreg=NULL;
    score=-1;
    vs=v;
...
//    return NULL;
  ...
  V2d Polygon2d::num(int i) {
    return vs.wrapnum(i);
  }

  bool Polygon2d::crosses(Line2d l) {
    for (int i=1;i<=vs.len;i++) {
...
//    return overlaps(&p);
  ...
  /* Skipping declared method  bool Polygon2d::overlaps(Polygon2d *p) */


  bool Polygon2d::overlaps(Polygon2d *p,float lenience) { // this has a bias towards being a container and not contained
  ...
  void Polygon2d::add(V2d v) {
    vs.add(v);
  }

  void Polygon2d::freedom() {
    vs.freedom();
  }
