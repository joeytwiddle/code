...
// Starts of class Region
  ...
  // Variable declared in .h file
  // Variable declared in .h file
  void Region::destroyallbut(void *n) {
...
//      printf("Nah!\n");
  ...
  void Region::destroyall() {
    destroyallbut(NULL);
  }
  void Region::destroyallbutmap() {
    destroyallbut(&map);
  }
  void Region::destroyallbutlist() {
    destroyallbut(&list);
  }
   Region::Region() { // If you make a list out of this, and then want to get the region, you must give it a width and height!
    map=NULL;
    list=NULL;
    cor=NULL;
    boundary=NULL;
   ...
   Region::Region(Polygon2d p,int w,int h) {
    map=NULL;
    list=NULL;
    width=w; height=h;
    cor=NULL;
...
//    destroy(list);
  ...
  float Region::bottommost() {
   ...
   Region *Region::listready() { // may as well use Region() instead
    Region *n=new Region();
    n->list=new List<Pixel>();
    return n;
  }
  List<Region *> *Region::oldsplit() { // Try splitting up using mean centre
    makelist();
    float l=leftmost();
    float r=rightmost();
    float t=topmost();
  ...
  void Region::add(Pixel v) {
    // makelist();
    list->add(v);
  }
  void Region::makelist() {
    if (list==NULL) {
      remakelist();
    }
//printf("Got list length %i\n",list->len);
  }
  void Region::remakelist() {
    // destroy(list); ?
    list=map->getlist();
  }
  void Region::remakemap(int width,int height) {
    if ((width<1) || (height<1))
...
//    printf("Making map (%i) %ix%i\n",list->len,width,height);
  ...
  Map2d<bool> *Region::getmap(int width,int height) {
    makemap(width,height);
    return map;
  }
  Map2d<bool> *Region::getmap() {
    return getmap(width,height);
  }
  void Region::makemap(int width,int height) {
    if (map==NULL)
  ...
  void Region::scaleto(int w,int h) {
    getsize();
//    printf("%i %i\n",width,height);
    makemap();
    if (width!=w || height!=h) {
...
//    return Region(getmap()->expand(r));
  ...
  List<int> Region::checkexpansion(Map2d<int> *regmap) {
    // Map version - speedy!
    makemap();
    makelist();
    List<int> l;
  ...
  List<int> Region::checkexpansion(Map2d<int> *regmap,int col) {
    // Map version - speedy!
    makemap();
    makelist();
    List<int> l;
...
/*    // List version - mem efficient but fucking slow!
  ...
  float Region::correlation() {
    makecorrelator();
    return cor->correlation();
  }
  void Region::makecorrelator() {
    if (cor==NULL)
      remakecorrelator();
  }
  void Region::remakecorrelator() {
    cor=new Correlator(getlist());
  }
  Correlator *Region::getcor() {
    makecorrelator();
    return cor;
  }
  /*
...
//    return atanf(sum);
  ...
  float Region::blength() {
    makecorrelator();
    return cor->length();
  }

  float Region::bheight() {
    makecorrelator();
    return cor->fatness();
  }

  Line2d Region::endtoend() { // finds ends from orientation
    return endtoend(ori());
  }

  Line2d Region::endtoend(V2d ori) {
    makelist();
    makecorrelator();
    float lowest=-1.0;
    float highest=-1.0;
  ...
  Line2d Region::endtoendvp(V2d vp) { // finds ends of line by distance from HVP
    // Assume VP far away
    V2d cen=centroid();
    V2d dir=(cen-vp).norm();
    return endtoend(dir);
  ...
  Line2d Region::line() { // uses centroid as center
...
//    printf("Made line length %f List %i\n",v.mod(),list->len);
  ...
  void Region::makeboundary() {
    if (boundary==NULL)
      remakeboundary();
  }
  Pixel Region::dir(int i) {
    float t=2.0*pi*(float)i/4.0;
    return Pixel(sin(t)*1.5,cos(t)*1.5);
  }
  void Region::remakeboundary() {
//printf("A\n");
    makelist();
//printf("B\n");
    makemap();
