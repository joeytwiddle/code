...
int Sinstr(String x,String s,int start) {
  for (int i=start-1;i<Slen(x)-Slen(s)+1;i++) {
    int j=0;
    while (x[i+j]==s[j]) {
      j++;
      if (j==Slen(s))
...
// Starts of class StringParser


  // Variable declared in .h file
  // Variable declared in .h file
   StringParser::StringParser(String s) {
    current=s;
    whole=s;
    // printf("Ready to parse %s\n",s);
  }
  void StringParser::error(String s) {
    printf("Whole %s\nAt %s\n%s\n",whole,current,s);
  }
  String StringParser::getbefore(String s) {
  ...
  bool StringParser::expect(String s) {
  ...
  void StringParser::expectorerr(String s) {
    if (!expect(s))
      error(Sformat("Expected %s",s));
  }
  char StringParser::getachar() {
    char c=current[0];
    current=Sfrom(current,2);
    return c;
  }
  String StringParser::getanyof(String ps,bool doerror) {
    String s="";
    bool stop=false;
    do {
  ...
  String StringParser::getanyof(String ps) {
