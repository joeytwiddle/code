#ifndef correlator2d_C
  #define correlator2d_C
...
// Starts of class Correlator2d


	// Variable declared in .h file
  // Variable declared in .h file // Points from which to find a line of best fit
  // Variable declared in .h file // Weightings of the points
  ...
  void Correlator2d::add(V2d v,float w) {
    points.add(v);
    weights.add(w);
    made=false;
  }
  void Correlator2d::add(V2d v) {
    add(v,1.0);
  }
  void Correlator2d::add(Pixel p) {
    add(V2d(p));
  }
  ...
  void Correlator2d::make() {
    if (!made)
      remake();
  }

  void Correlator2d::addposs(List<int> n,List<List<int> > *poss,List<V2d> *cens,List<float> *totals) {
    poss->add(n);
        float total=0;
        V2d cen=V2d(0,0);
...
//     printf("Trying %s with %i\n",ps.toString(),i);
  ...
  float Correlator2d::totalfor(List<int> ps) {
    float total=0;
  ...
  V2d Correlator2d::centroidfor(List<int> ps) {
    V2d cen=V2d(0,0);
    for (int i=0;i<ps.len;i++) {
      int j=ps.get(i);
  ...
  boolean Correlator2d::tryset(List<int> ps) {
    return tryset(ps,centroidfor(ps),totalfor(ps));
  }

  boolean Correlator2d::trysetJoeysMode(List<int> ps,V2d cen,float total) {
    float changed=false;
    // Try all angles
...
//      bestposs=i;
  ...
  boolean Correlator2d::trysetPerpMode(List<int> ps,V2d cen,float total) {
		float sumx=0;
		float sumxsqd=0;
		float sumy=0;
		float sumysqd=0;
	...
	float Correlator2d::errPerpMode(List<int> ps,float a,float b) {
		float err=0;
		for (int i=0;i<ps.len;i++) {
			V2d *v=points.p2num(i+1);
  ...
  boolean Correlator2d::tryset(List<int> ps,V2d cen,float total) {
		if (mode==joeysMode)
  ...
  float Correlator2d::angle() {
    make();
    return bestang;
  }
  float Correlator2d::error() {
    make();
    return besterr;
  }
  Line2d Correlator2d::line() {
    make();
    V2d ori=V2d::angle(bestang);
    return Line2d(centroid,centroid+ori);
  }

  float Correlator2d::crossesy() {
    make();
    Line2d l=line();
  ...
  void Correlator2d::remakebruteforce() { // Pretty useless because exponential
...
/*      for (int i=0;i<points.len;i++) {
    ...
    bool Correlator2d::used(int i) { /* i from 1..size */
