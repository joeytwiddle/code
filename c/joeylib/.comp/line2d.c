...
// Starts of class Line2d
	...
	void Line2d::swapends() {
		V2d tmp=a;
		a=b;
		b=tmp;
	}

	V2d Line2d::end(int i) {
		if (i==1)
			return a;
		if (i==2)
			return b;
	...
	Line2d Line2d::operator+(V2d v) {
		return Line2d(a+v,b+v);
	}
	Line2d Line2d::operator*(float f) {
		return Line2d(a*f,b*f);
	}

	Line2d Line2d::expandedBy(float dist) {
		V2d dir=(b-a).normalised();
		return Line2d(a-dir*dist,b+dir*dist);
	}
	Line2d Line2d::expandedForever() {
		V2d len=(b-a);
		float bignum=100000000.0/len.mag();
		return expandedBy(bignum);
	}
	Line2d Line2d::expandedTo(float wid) {
		float len=(b-a).mag();
		float diff=(wid-len)/2.0;
		return expandedBy(diff);
	}

	float Line2d::xaty(float y) {
		float t=(y-a.y)/(b.y-a.y);
		V2d p=a+t*(b-a);
		return p.x;
	}
	float Line2d::yatx(float x) {
		float t=(x-a.x)/(b.x-a.x);
		V2d p=a+t*(b-a);
		return p.y;
	}

	V2d Line2d::perpproject(V2d v) { // Finds the closest point on the line to v
		float thru=((b-a).normalised()).dot(v-a);
		return a+thru*((b-a).normalised());
		// return intersection(Line2d(v,v+(b-a).perp());
	}

	float Line2d::findintersectionnum(Line2d *o) {
	...
	V2d *Line2d::findintersectionornull(Line2d o) {
	...
	V2d Line2d::findintersection(Line2d o) {
		//    printf("Finding intersection of %s with %s\n",toString(),o.toString());
		float l=findintersectionnum(o);
		//    printf("Num %f Pos %s\n",l,(a+l*(b-a)).toString());
		return a+l*(b-a);
	}
	V2d Line2d::intersection(Line2d o) {
		return findintersection(o);
	}
	V2d Line2d::intersect(Line2d o) {
		return findintersection(o);
	}
	bool Line2d::crosses(Line2d *o) {
		//    //flibble(*this,2);
		//    //flibble(*o,3);
		float l=findintersectionnum(o);
	...
	bool Line2d::crosses(Line2d o) {
	 ...
	 V2d Line2d::intersection(Line2d la,Line2d lb) {
	...
	String Line2d::toString() {
	 ...
	 float Line2d::getlen(Line2d l) {
		return l.length;
	}

	// Set intersection point only if lines are not parallel
	void Line2d::setintifnotpar(Line2d ol,V2d *in) {
	...
	void Line2d::clipbyrectangle(float l,float t,float r,float bo) {
...
// Starts of class QuadsLine2d

	
		// Don't know what these were for!  Some PhD stuff no doubt.
		// Variable declared in .h file//=List<List<Line2d *> >();//2,List<Line2d *>());
		// Variable declared in .h file//=List<List<int> >(2,List<int>());
		// Variable declared in .h file

		 QuadsLine2d::QuadsLine2d() {
			allusedup=false;
		}
		 QuadsLine2d::QuadsLine2d(V2d aa,V2d bb) {
			allusedup=false;
			a=aa;
			b=bb;
			refresh();
		...
		QuadsLine2d QuadsLine2d::operator*(float f) {
