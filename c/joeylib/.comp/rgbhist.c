...
// Starts of class RGBHistogram
  ...
  // Variable declared in .h file
   ...
   RGBHistogram::RGBHistogram(int q) {
    quant=q;
    numsamps=0;
    rgb=new (uchar**)[q];
    for (int i=0;i<q;i++) {
  ...
  float RGBHistogram::correlation() {
...
//    printf("(%f) ",tmp);
  ...
  void RGBHistogram::addocc(myRGB *c) {
  ...
  RGBHistogram *RGBHistogram::normsmoothed(int radius) {
    RGBHistogram *n=new RGBHistogram(quant);
    for (int i=0;i<quant;i++)
    for (int j=0;j<quant;j++)
    for (int k=0;k<quant;k++) {
  ...
  // Gaussian
  RGBHistogram *RGBHistogram::smoothed(int radius) {
    RGBHistogram *n=new RGBHistogram(quant);
   ...
   float RGBHistogram::difference(RGBHistogram a,RGBHistogram b) {
    if (a.quant!=b.quant)
  ...
  int RGBHistogram::maximumat(int i,int j,int k) {
    int rad=2;
  ...
  int RGBHistogram::eightmaximumat(int i,int j,int k) {
    int tmp=rgb[i][j][k];
    for (int x=-1;x<=1;x+=2)
    for (int y=-1;y<=1;y+=2)
    for (int z=-1;z<=1;z+=2)
  ...
  int RGBHistogram::fourmaximumat(int i,int j,int k) {
