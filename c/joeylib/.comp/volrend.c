...
// Starts of class VolRend
  ...
  V3d VolRend::searchline(V3d front,V3d back,bool wanted) {
    V3d t,mid;
    t=(front-back);
    if (t.mod()<smallest)
      return back;
  ...
  float VolRend::getdepth(int i,int j) {
    if (!rend->inmap(i,j))
      return back;
  ...
  int VolRend::getcol(int rx,int ry) {
    int col;
  ...
  void VolRend::fillsquare(int x,int y,int w,int h) {
    #ifdef ALLEGRO
  if (key[KEY_SPACE])
    exit(0);
    #endif
  int nwcol=getcol(x,y);
   ...
   VolRend::VolRend(int w,int h,bool (*f)(V3d),int cdt,float jump,int s,float acc,float ba,int r) {
  crazy=-12345.6789;
    scrwid=w;
    scrhei=h;
    b=new JBmp(w,h);
