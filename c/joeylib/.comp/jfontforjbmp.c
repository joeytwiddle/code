#ifndef jfontforjbmp_C
  #define jfontforjbmp_C

  #include <jfontforjbmp.h>
...
// Starts of class FontBit
   ...
   void FontBit::writetojbmp(JBmp *j,int x,int y,float scale,int c) {
    printf("arse\n");
  }
// End class 
...
// Starts of class FontChar
  ...
  void FontChar::writetojbmp(JBmp *j,int x,int y,float scale,int c) {
...
// Starts of class FLine
  ...
  void FLine::writetojbmp(JBmp *j,int x,int y,float scale,int c) {
    if (round)
...
//      printf("Rounded ");
  ...
  void FArc::writetojbmp(JBmp *j,int cx,int cy,float scale,int c) {
    V2d o,i;
    if (ab<aa)
      ab+=2*pi;
...
// Starts of class JFont
  ...
  void JFont::writechar(String s,JBmp *j,int x,int y,float scale,int c) {
  ...
  void JFont::writeString(String s,JBmp *j,int x,int y,float scale,int c) {
  ...
  void JFont::centerString(String s,JBmp *j,int y,float scale,int c) {
    int x=j->width/2-scale*1.5*(Slen(s)-1)/2.0;
    writeString(s,j,x,y,scale,c);
  }
  void JFont::centerString(String s,JBmp *j,int cx,int y,float scale,int c) {
