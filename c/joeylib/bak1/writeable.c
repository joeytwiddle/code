...
public:
  ...
  virtual void setpos(int i,int j,int c) {
    error("Write::setpos(int) not cast before write");
  }
  virtual void setpos(int i,int j,myRGB c) {
...
/*  template <class CT>
  ...
  template <class CT>
  void setpos(Pixel p,CT c) {
    setpos(p.x,p.y,c);
  }
  template <class CT>
  void setpixel(int x,int y,CT c) {
    setpos(x,y,c);
  }
  template <class CT>
  void setpixel(Pixel p,CT c) {
    setpos(p.x,p.y,c);
  }
...
//      printf("%sDidn't write line because got %s when clipped\n",s,l.toString());
  ...
  template <class CT>
  void line(Pixel a,Pixel b,CT c) {
  // #ifndef UNIX
    line(a.x,a.y,b.x,b.y,c);
  // #endif
  }
  template <class CT>
  void line(V2d a,V2d b,CT c) {
  // #ifndef UNIX
    line(a.x,a.y,b.x,b.y,c);
  // #endif
  }
  ...
  template <class CT>
  void cross(V2d cen,int s,CT c) {
    line(cen+V2d(s,s),cen-V2d(s,s),c);
    line(cen+V2d(-s,s),cen-V2d(-s,s),c);
  }

  template <class CT>
  void thickcross(V2d cen,int s,float wid,CT c) {
    thickline(cen+V2d(s,s),cen-V2d(s,s),c,wid);
    thickline(cen+V2d(-s,s),cen-V2d(-s,s),c,wid);
  }

  template <class CT>
  void vline(int x,int ya,int yb,CT c) {
    float atob;
    bool done=false;
    int s=sgn(yb-ya);
...
//    error("Writeable::vthickline not cast before write");
  ...
  template <class CT>
  void rthickline (V2d a, V2d b, float w, CT c) {
    V2d a2b=w/2.0*(b-a).norm();
    V2d p=a2b.perp();
...
//    error("Writeable::filltri not cast before write");
  ...
  template <class CT>
  void filledcircle(V2d cen,float r,CT c) {
    filledcircle(cen.x,cen.y,r,c);
  }

  template <class CT>
  void filledcircle(int cx,int cy,float r,CT c) {
    int x,h,y,px,py;
    for (x=-r;x<=r;x++) {
...
/*  template <class CT>
  ...
  virtual void plotandfree(List<Pixel> ps,int c) {
    for (int i=1;i<=ps.len;i++)
      setpos(ps.num(i),c);
    ps.freedom();
  }

  virtual void plotandfree(List<Pixel> ps,myRGB c) {
    for (int i=1;i<=ps.len;i++)
  ...
  void cross(V2d cen,float s,ColType c) {
    line(cen+V2d(-s,-s),cen+V2d(s,s),c);
    line(cen+V2d(-s,s),cen+V2d(s,-s),c);
  }
