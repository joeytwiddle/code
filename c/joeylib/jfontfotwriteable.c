...
class FontBit {
public:
  virtual void writetojbmp(Writeable *j,int x,int y,float scale,int c) {
    error("FontBit::writejbmp executed without cast");
  }
  virtual void writetojbmp(Writeable *j,int x,int y,float scale,myRGB c) {
    error("FontBit::writejbmp executed without cast");
  }
};
...
public:
  ...
  void writetojbmp(Writeable *j,int x,int y,float scale,Colour c) {
...
//    printf("new arc %f %f %f %f %f\n",x,y,r,aa,ab);
  ...
  FArc(float a,float b,float c,float d,bool rn) {
    x=a;
    y=b;
    r=c;
    w=d;
...
public:
  ...
  template <class Colour>
  void writechar(String s,Writeable *j,int x,int y,float scale,Colour c) {
  ...
  template <class Colour>
  void writeString(String s,Writeable *j,int x,int y,float scale,Colour c) {
  ...
  template <class Colour>
  void centerString(String s,Writeable *j,int y,float scale,Colour c) {
    int x=j->width/2-scale*1.5*(Slen(s)-1)/2.0;
    writeString(s,j,x,y,scale,c);
  }
  template <class Colour>
  void centerString(String s,Writeable *j,int cx,int y,float scale,Colour c) {
