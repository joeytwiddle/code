...
public:
  ...
  RGBmp(int w,int h,myRGB r) {
    make(w,h);
    clear(r);
  }

  myRGB getpos(int x,int y) {
    if (inbmp(x,y))
      return bmp[x][y];
    else
      return myRGB::random();
...
//    printf("New RGBmp created.\n");
  ...
  Map3d<int> *houghtransform(int angquant,int xquant,int yquant,int spacerad) {
  ...
  Map2d<float> *getedges(int maskrad) {
  ...
  void setrectangle(int left,int top,int wid,int hei,myRGB r) {
  ...
  void thickline(Line2d l,myRGB r,float w) {
    thickline(l.a,l.b,r,w);
  }
  void thickline(V2d a,V2d b,myRGB r,float w) {
  ...
  void thicklineinf(V2d a,V2d b,float w,myRGB r) {
    Line2d l=Line2d(a,b).expandedForever();
    l.clipbyrectangle(0,0,width-1,height-1);
    thickline(l,r,w);
  }

  void filltri(V2d a,V2d b,V2d c,myRGB r) { // inefficient
    intertri(a.x,a.y,b.x,b.y,c.x,c.y,r.v3d(),r.v3d(),r.v3d());
  }

  void line(Line2d l,myRGB r) {
    line(l.a,l.b,r);
  }
  void line(V2d a,V2d b,V3d v) {
    line(a.x,a.y,b.x,b.y,&v.rgb());
  }
  void line(V2d a,V2d b,myRGB r) {
    line(a.x,a.y,b.x,b.y,&r);
  }
  void line(Pixel a,Pixel b,myRGB r) {
    line(a.x,a.y,b.x,b.y,&r);
  }
  void line(int x1, int y1, int x2, int y2,myRGB *r) {
...
/* Original for RGBmp
  ...
  boolean valid(int x,int y,Map2d<int> *chosen,myRGB *r,float tol) {
    if (!inimage(x,y))
      return false;
    if (chosen->pos[x][y]>0)
      return false;
  ...
  List<List<Pixel> > *getfilledregions(float tolerance) {
  ...
  List<List<Pixel> > *oldclassifyscattered(myRGB preferred,float tolerance,int scatrad) {
  ...
  List<List<Pixel> > *classifyallscattered(float tolerance,int scatrad) {
  ...
  List<List<Pixel> > *classifyscattered(myRGB preferred,float tolerance,int scatrad) {
  ...
  void display(int winwid,int winhei) {
    #ifdef ALLEGRO
  #ifndef NOGRAPHICS
  ...
  // attempt for DOS
  RGBmp(String fname) { //- depreceated, use static RGBmp::readfile(String fname)
...
/*    FILE *fp;
  ...
  static RGBmp *readfile(String s) {
  ...
  static RGBmp *readppm(String filename) {
    unsigned char *data;
    int *height,*width;
  ...
  static RGBmp *readbmp(String fname) {
    RGBmp *r;
  ...
  Map2d<float> *getgreyscale() { // Should really use YUV
  ...
  Map2d<RGBHistogram> *getlocalhistmap(int histrad,float scale) {
  ...
  static RGBmp *toRGBmp(Map2d<float> *m) {
  ...
  template <class Obj>
  static RGBmp *toRGBmp(Map2d<Obj> *m) {
    Map2d<float> *f=m->getgreyscale(); // m->applyfn(&tofloat);
    return toRGBmp(f);
  }


  void setupgrid(Map2d<V2d> *grid,int left,int top,int size,V2d hvp,V2d vvp) {
    int hsize=size/2;
  ...
  RGBmp *recoverquadrilateral(V2d a,V2d b,V2d c,V2d d,int imwidth) {
...
*/
    ...
    myRGB getinter(float x,float y) {
      int lx=(int)x;
      int ly=(int)y;
      float e=x-lx;
      float s=y-ly;
...
*/
  ...
  RGBmp *linearrecoverquad(List<V2d> *ls,float scale,int imwid) {
    List<V2d> l;
    for (int i=1;i<=4;i++)
  ...
  RGBmp *recoverquad(List<V2d> *ls,float scale,int imwid) {
...
//    printf("%s=%s %s=%s %s=%s %s=%s\n",ia.toString(),a.perpproj().toString(),ib.toString(),b.perpproj().toString(),ic.toString(),c.perpproj().toString(),id.toString(),d.perpproj().toString());
  ...
  void setpos(int x,int y,V3d c) {
    setpos(x,y,c.rgb());
  }

  void setpos(int x,int y,myRGB c) {
    if (inbmp(x,y))
      bmp[x][y]=c;
  }

  void setpixel(int x,int y,V3d c) {
    setpos(x,y,c);
  }

  void setpixel(int x,int y,myRGB c) {
    if (inbmp(x,y)) {
...
//      printf("Setting %i %i (%i,%i)\n",x,y,width,height);
  ...
  void setpixel(Pixel p,myRGB c) {
    setpixel(p.x,p.y,c);
  }

  void setpos(Pixel p,myRGB c) {
    setpixel(p.x,p.y,c);
  }

  void intvline(int x,int ya,int yb,V3d ca,V3d cb) {
    float atob;
    bool done=false;
    int s=sgn(yb-ya);
  ...
  void intvline(int x,int ya,int yb,V3d ca,V3d cb,Texture *t) {
    float atob;
    bool done=false;
    int s=sgn(yb-ya);
    for (register int i=ya;!done;i+=s) {
  ...
  void intertri(int xa,int ya,int xb,int yb,int xc,int yc,V3d ca,V3d cb,V3d cc,Texture *t) {
    if (xa>xb) {
  ...
  void intertri(int xa,int ya,int xb,int yb,int xc,int yc,myRGB ca,myRGB cb,myRGB cc) {
    intertri(xa,ya,xb,yb,xc,yc,ca.v3d(),cb.v3d(),cc.v3d());
  }

  void intertri(V2d a,V2d b,V2d c,myRGB ca,myRGB cb,myRGB cc) {
    intertri(a.x,a.y,b.x,b.y,c.x,c.y,ca.v3d(),cb.v3d(),cc.v3d());
  }

  void intertri(int xa,int ya,int xb,int yb,int xc,int yc,V3d ca,V3d cb,V3d cc) {
    if (xa>xb) {
      swap(&xa,&xb); swap(&ya,&yb);
      swap(&ca,&cb);
    }
  ...
  bool inbmp(int x,int y) {
    return (x>=0 && x<width && y>=0 && y<height);
  }

  RGBmp *scaletowidth(int w) {
    return scale(this,(float)w/(float)width);
  }

  RGBmp *scaledby(float s) {
    return scale(this,s);
  }

  myRGB getposinterpolate(float x,float y) {
    int l=x;
    int t=y;
    float i=x-(float)l;
    float j=y-(float)t;
  ...
  static RGBmp *scale(RGBmp *in,float scale) {
  ...
  static RGBmp *fromv3ds(Map2d<V3d> *m) {
  ...
  void line(Line2d *l,myRGB c) {
    line(l->a,l->b,c);
  }
  void sthickline(V2d a,V2d b,float w,myRGB c) {
    V2d a2b=w/2.0*(b-a).norm();
    V2d p=a2b.perp();
    filltri(a+p,a-p,b+p,c);
    filltri(a-p,b+p,b-p,c);
  }
  void thickline(V2d a,V2d b,float w,myRGB c) {
    V2d a2b=w/2.0*(b-a).norm();
    V2d p=a2b.perp();
    filltri(a-a2b+p,a-a2b-p,b+a2b+p,c);
    filltri(a-a2b-p,b+a2b+p,b+a2b-p,c);
  }
  void thickline(int xa,int ya,int xb,int yb,float w,myRGB c) {
    thickline(V2d(xa,ya),V2d(xb,yb),w,c);
  }
  void thickline(Line2d l,int w,myRGB c) {
    thickline(l.a,l.b,w,c);
  }
  void thicklinev(V2d a,V2d b,float w,myRGB c) {
    V2d p=V2d(w/2.0,0);
    V2d a2b=V2d(0,w/2.0)*mysgn(b.y-a.y);
    filltri(a-a2b+p,a-a2b-p,b+a2b+p,c);
    filltri(a-a2b-p,b+a2b+p,b+a2b-p,c);
  }
  void thicklineh(V2d a,V2d b,float w,myRGB c) {
    V2d p=V2d(0,w/2.0+1);
    V2d a2b=V2d(w/2.0-1,0)*mysgn(b.x-a.x);
    filltri(a-a2b+p,a-a2b-p,b+a2b+p,c);
    filltri(a-a2b-p,b+a2b+p,b+a2b-p,c);
  }
  void rthickline(V2d a,V2d b,float w,myRGB c) {
    V2d a2b=w/2.0*(b-a).norm();
    V2d p=a2b.perp();
    filltri(a+p,a-p,b+p,c);
    filltri(a-p,b+p,b-p,c);
...
void opencircle(Pixel p,float r,myRGB c) {
  opencircle(p.x,p.y,r,c);
}
void opencircle(V2d v,float r,myRGB c) {
  opencircle(v.x,v.y,r,c);
}
void opencircle(int x,int y,float r,myRGB c) {
  V2d last=V2d(x,y+r);
  int steps=4+r/2;
  ...
  void filledcircle(int cx,int cy,float r,myRGB c) {
    int x,h,y,px,py;
    for (x=-r;x<=r;x++) {
  ...
  void cross(V2d cen,float s,myRGB c) {
    line(cen+V2d(-s,-s),cen+V2d(s,s),c);
    line(cen+V2d(-s,s),cen+V2d(s,-s),c);
  }
  void cross(float x,float y,float s,myRGB c) {
    line(V2d(x-s,y-s),V2d(x+s,y+s),c);
    line(V2d(x-s,y+s),V2d(x+s,y-s),c);
  }
  void filledcircle(V2d pos,float r,myRGB c) {
    filledcircle(pos.x,pos.y,r,c);
  }
  RGBmp *applyfn(myRGB fn(myRGB)) {
    RGBmp *n=new RGBmp(width,height);
    for (int i=0;i<width;i++)
    for (int j=0;j<height;j++)
