...
void newpart(int i) {
  ...
  if (p>pgwell && p<pgwell+ptwist) {
    part[i].twist=1;
    part[i].twistaxis=V3drandom();
    part[i].twistpow=myrnd(-.1,.1);
    twist[twists]=i; twists++;
    printf("twist %f ",part[i].twistpow); V3dprint(part[i].twistaxis); printf("\n");
  } else
    part[i].twist=0;
}
...
int plotpart2(Part p,float eyex,int c) {
  ...
  if (abs(scrx-scrwid/2)>scrwid/2 || abs(scry-scrhei/2)>scrhei/2) {
    ...
    if (p.twist) {
      findplanehit(newV3d(eyex,0,0),V3dadd(p.pos,p.twistaxis),planedist,&x,&y);
...
void movepart(int i) {
  ...
  if (part[i].gwell) {
    ...
    for (j=0;j<gwells;j++) {
      ...
      if (gwell[j]!=i)
        part[i].vel=V3dadd(part[i].vel,V3dmult(part[gwell[j]].gwellpow/(V3ddist(part[gwell[j]].pos,part[i].pos)+1)*gwellstr,V3dnorm(V3dsub(part[gwell[j]].pos,part[i].pos))));
    }
    // Twisted by twisters
    for (j=0;j<twists;j++) {
      ...
      if (twist[j]!=i)
        part[i].pos=V3dadd(part[twist[j]].pos,V3drotate(V3dsub(part[i].pos,part[twist[j]].pos),part[twist[j]].twistaxis,part[twist[j]].twistpow*twiststr/mysquare(V3ddist(part[i].pos,part[twist[j]].pos))));
    }
  }
  // Damping
