...
//  gaus=a.floatafter("-gh","gaussian hillock",10.0);
  bool writeframes=a.argexists("-wf","write frames to BMPs");
  a.done();
...
//      x.y=x.y*ramp(pythag((float)i/(float)size,(float)j/(float)size)/pythag(1,1)*2.0);
      ...
      float snow=0.5;
      float green=0;
      V3d col=( x.y>snow ? V3d(1,1,1) :
                x.y>green ? pull(V3d(0,0.5,0),(x.y-green)/(snow-green),V3d(1,1,1)) :
                x.y>waterlevel ? V3d(0,0.5,0) : V3d(0,0,0.7) );
      bool wat;
      if (x.y<waterlevel) {
        x.y=waterlevel;
        wat=true;
...
//      x=mb*x;
      ...
      if (i>0 && j>0) {
        o.add(new Gouraud(&vs.pos[i][j],&vs.pos[i-1][j],&vs.pos[i][j-1]));
        o.add(new Gouraud(&vs.pos[i-1][j],&vs.pos[i-1][j-1],&vs.pos[i][j-1]));
      }
    }
    ...
    if (writeframes)
    rgbmp.writefile(getnextfilename("mnt","bmp"));
