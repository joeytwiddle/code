...
void main() {
  ...
  Quaternion offset=Quaternion(0,0,0,0);
  do {
    #define R nicernd(0.1)
    offset=offset+Quaternion(R,R,R,R);
    for (int i=0;i<scrwid;i+=jump) {
      for (int j=0;j<scrhei;j+=jump) {
        ...
        current=Quaternion(left+2.0*wid*(float)i/(float)scrwid,top+2.0*hei*(float)j/(float)scrhei,0,0);
        b->setrectangle(i,j,jump,jump,julquat(offset));
        if (key[KEY_SPACE])
          exit(0);
      }
      b->writetoscreen();
