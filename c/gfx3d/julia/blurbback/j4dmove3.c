...
float rotation;
  ...
  int frames=30;
  int maxiter=10; // Set very low - we are seeing an early shell of the Julia set
...
int main(int argc, char **argv) {
  ...
  for (int frame=0;frame<=frames;frame++) {
    ...
    float dslice=0.5*(thru);
    rotation=-pi/3.0+pi/8.0*thru;

    // Random c
//    current=Quaternion(floatrnd(-.5,.5),floatrnd(-.5,.5),floatrnd(-.5,.5),floatrnd(-.5,.5));
