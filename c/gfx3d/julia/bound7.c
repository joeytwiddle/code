...
// Boundary tracing (using list) and rendering

/* All my source code is released under the GNU public licence.
   I do not accept responsibility for unintentional bad side effects.
...
float rotation=-pi/3.5;

float smallest=0.001;
...
Map3d<uchar> *map=new Map3d<uchar>(spacequant,spacequant,spacequant,(uchar)0);
Quaternion current;
Map2d<float> *rend;
...
Quaternion maptoqspace(int x,int y,int z) {
  V3d tmp=*new V3d(left+(float)x*2.0*wid/(float)spacequant,top+(float)y*2.0*hei/(float)spacequant,front+(float)z*2.0*dep/(float)spacequant);
  tmp=V3d::rotate(tmp,V3d(0,1,0),rotation);
  return Quaternion(tmp.x,tmp.y,tmp.z,dslice);
}
...
void render(int rx,int ry) {
  if (rx>0 && ry>0 && rx<rend->width && ry<rend->height) {
    int col;
    if (rend->pos[rx][ry]==back && rend->pos[rx+1][ry]==back && rend->pos[rx][ry+1]==back)
      col=0;
    else {
...
/*  int z=0;
  do {
    ...
    // printf("%i,%i,%i : ",x,y,z);
    // r=stateat(x,y,z);
  } while (!isboundary(x,y,z) && z<spacequant-1);*/
  // From back to front
  int z=spacequant-1;
  do {
    z--;
  ...
  while (l.len>0) {
    ...
    if (i!=0 || j!=0 || k!=0) {
      ...
      if (dx>=1 && dx<spacequant-1 && dy>=1 && dy<spacequant-1 && dz>=1 && dz<spacequant-1)
        ...
        Quaternion q=searchline(maptoqspace(dx,dy,dz),maptoqspace(dx,dy,dz-1));
        V3d tmp=*new V3d(q.a,q.b,q.c);
        tmp=V3d::rotate(tmp,V3d(0,1,0),-rotation);
        float rz=tmp.z;
    ...
    // printf("%i:",depth--);
    putpixel(screen,x/2,y/2,255.0*(1-(float)z/(float)spacequant));
  }
  while (!key[KEY_SPACE]) { 
  }
  allegro_exit();
