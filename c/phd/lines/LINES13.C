...
//      bpols.add(Polar(bpixs.num(i)));
  ...
  Blob(int c,int bi) { // A new blob based on one that has got hits
    Blob *b=bs.p2num(bi);
    col=c;
    blobbed=false;
  ...
  int gettype() { // *** If contains 2+ sentences then Paragraph
    if (type==Undefined) {
      ...
      for (int i=1;i<=sbs.len;i++) {
        int t=bs.p2num(sbs.num(i))->gettype();
        if (t==Base)
          basecnt++;
        if (t==Sentence) {
          sentcnt++;
...
// setpos error      plotandfree(m,Pixel::line(centroid-o,centroid+o),r);
  ...
  V2d ori() {
    ...
    for (int i=1;i<=sbs.len;i++) {
      int cnt=bs.p2num(sbs.num(i))->area;
      V2d ctr=bs.p2num(sbs.num(i))->centroid;
      for (int j=1;j<=cnt;j++)
        c.add(ctr);
    }
    V2d ori=c.ori();
...
//    cntsent++;
  for (int i=1;i<=hits.len;i++) {
    ...
    n.add(a);
//    if (c->gettype()==Sentence)
//      cntsent++;
  }
//  if (cntsent>=2)
//    n.type=Paragraph; // *** Doesn't work!
//  if (cntsent>=1)
//    printf("Joined %i lines\n",cntsent);
