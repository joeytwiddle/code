...
#define LineSpace 2.0 // the distance between lines in the paragraph
...
//    r=myRGB::hue(myrnd());
    for (int i=1;i<=bases.len;i++) {
      ...
      m->line(Pixel(b->centroid)+Pixel(-5,0),Pixel(b->centroid)+Pixel(+5,0),myRGB::blue);
      m->line(Pixel(b->centroid)+Pixel(0,-5),Pixel(b->centroid)+Pixel(0,+5),myRGB::blue);
    }
...
//        printf("Plotting lines...");
        ...
        // Doing linear correlation with line area
/*        float scale=1.0/20.0;
        for (int i=1;i<=b->lines.len;i++) {
          V2d cen=bs.p2num(b->lines.num(i))->centroid;
          float x=(cen-l.a).dot((l.b-l.a).norm());
          float y=bs.p2num(b->lines.num(i))->area; // Area of region! (used to be ->height())
          printf("%f , %f\n",x,y);
          out.cross(l.a+(l.b-l.a).norm()*x+lineori*y*scale,5,myRGB::white);
          c2.add(x,y);
        }*/

        // Doing linear correlation with line (character) height
        float scale=5.0;
        for (int i=1;i<=b->lines.len;i++) {
          V2d cen=bs.p2num(b->lines.num(i))->centroid;
          float x=(cen-l.a).dot((l.b-l.a).norm());
