...
//    printf("VP %s l=%.2f r=%.2f\n",pos.toString(),left,right);
  ...
  void add(V2d v) { // accepts a +- vector relative to centre of image, with ma
g<=width above
...
v.toString(),l,pp.width,left,ang,right,q);
    }
  }
...
//      printf("Circle=%s\n",pos.toString());
      ...
      if (pos.mag()<1.0) {
        ...
        // Must lie at some distance (PP boundary not accurate near or inside i
mage)
...
//    printf("Angle up middle %f with centre %s\n",ang,ave.toString());
    ...
    if (doinfo) {
      ...
      info.line(baseline.a-diff,baseline.a+diff,myRGB::black);
      // Plot correlation points
      for (int i=1;i<=cens.len;i++) {
        info.cross(leftc.points.num(i).x,leftc.points.num(i).y,5,myRGB::red);
        if (leftc.used(i))
          info.opencircle(leftc.points.num(i).x,leftc.points.num(i).y,5,myRGB::
red);
...
// info.opencircle(cenc.points.num(i).x,cenc.points.num(i).y,5,myRGB::red);
        ...
        if (rightc.used(i))
          info.opencircle(rightc.points.num(i).x,rightc.points.num(i).y,5,myRGB
::red);
      }
    }
...
//  bool invert=a.argexists("-inv","invert input image to look for line spaces"
  ...
  ProjectionProfiler pp=ProjectionProfiler(cirres,ppres,binimg,writepps,writebe
stpp,doinfo);
  pp.setup();
  pp.calculate();
  V2d vp=pp.getvp();
  ...
  if (recoverquad) {
    ...
    float ha=vp.realleft+(vp.right-vp.left)*(float)vp.pp.firstnonzero()/(float)
vp.pp.width-henlarge;
    float hb=vp.realleft+(vp.right-vp.left)*(float)vp.pp.lastnonzero()/(float)v
p.pp.width+henlarge;
