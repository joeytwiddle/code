...
//    float farthest=maxinfcir*1.1e2;
  ...
  void getgapangles() {
    ...
    PPunit slice=pp.averageCropped();
    // PPunit slice=top;
...
//    printf("Angle up middle %f with centre %s\n",ang,ave.toString());
    ...
    if (doinfo) {
      drawCorrelator2dInColour(cs).writefile("spacings.bmp");
...
//  bool invert=a.argexists("-inv","invert input image to look for line spaces");
  ...
  V2d newvvp=vvpfromhvpandbaseline(hvp,pp.baseline,eye);
...
//      origimage.thickline(tl,hvp,1,myRGB::blue);
//      origimage.thickline(bl,hvp,1,myRGB::blue);

    }

    origimage.thickline(tl,tr,4,myRGB::blue);
    origimage.thickline(br,tr,4,myRGB::blue);
    origimage.thickline(br,bl,4,myRGB::blue);
    origimage.thickline(tl,bl,4,myRGB::blue);
