...
//    printf("Angle up middle %f with centre %s\n",ang,ave.toString());
    ...
    if (doinfo) {
      ...
      drawCorrelator(cenc).writefile("corcentre.bmp");
      drawCorrelator(leftc).writefile("corleft.bmp");
      drawCorrelator(rightc).writefile("corright.bmp");
      ...
      for (int i=1;i<=cens.len;i++) {
        info.cross(leftc.xs.num(i),leftc.ys.num(i),5,myRGB::red);
        info.cross(cenc.xs.num(i),cenc.ys.num(i),5,myRGB::red);
        info.cross(rightc.xs.num(i),rightc.ys.num(i),5,myRGB::red);
      }
    }
    ...
    for (int i=1;i<=cdsend;i++) {
      ...
      if (correlator==LineSpacing) {
        ...
        float b=cds.num(i+1);
        float pos=(a+b)/2.0;
        cs.add(pos,myabs(b-a),weight);
      } else if (correlator==LineWidth) {
        // y = width of line
        float pos=a;
        cs.add(pos,lines.num(i).length);
      } else {
...
printf("l\n");
    if (doinfo) {
      drawCorrelator(cs).writefile("spacings.bmp");
...
//    float enlarge=pi/200.0;
    float enlarge=0.0;
    float ha=vp.realleft+(vp.right-vp.left)*(float)vp.pp.firstnonzero()/(float)vp.pp.width-enlarge;
    float hb=vp.realleft+(vp.right-vp.left)*(float)vp.pp.lastnonzero()/(float)vp.pp.width+enlarge;
