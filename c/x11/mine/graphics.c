...
/*	XPutImage(display,toplevel,gc, */


	myscreen = DefaultScreen(display);				
	defaultVisual = DefaultVisual(display,myscreen);

	width = top_width;
	height = top_height;

  buffer_16bpp = (char *) malloc ((width) * (height) * 2);
  buffer_32bpp = (char *) malloc ((width) * (height) * 4);
	
	ximage=XCreateImage(display,defaultVisual,1,XYBitmap,0,(char *)buffer_16bpp,width,height,16,width*2);

	ximage->data=buffer_16bpp;
