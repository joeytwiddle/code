...
BasicWin BWCreateNewWindow(display, argc, argv)
Display *display;
int argc;
char **argv;
{
...
static get_graphical_GC(win, gc, font_info, screen_num, display)
Window win;
GC *gc;
...
static load_font(font_info, display)
XFontStruct **font_info;
Display *display;
{
	char *fontname = "9x15";
...
void BWCloseWindow(this_window)
...
void BWPrintText(this_window, text, x, y)
...
void BWDrawLine(this_window, sx, sy, ex, ey)
...
void BWDrawRectangle(this_window, sx, sy, width, height)
...
void BWDrawPoint(this_window, x, y)
...
void BWClearArea(this_window, sx, sy, width, height)
