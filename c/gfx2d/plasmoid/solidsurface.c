...
void main() {
  ...
  do {
    ...
    for (int x=0;x<scrwid && !key[KEY_ESC];x+=jump) {
      for (int y=scrhei-1;y>=0;y-=jump) {
        int xa,ya,xb,yb,xc,yc,xd,yd;
        if (intmap.pos[x][y]==0)
          calcintmap(x,y);
        if (x>=jump && y>=jump) {
          PPgetscrpos(hillfor(x-jump,y-jump),&xa,&ya);
          PPgetscrpos(hillfor(x,y-jump),&xb,&yb);
          PPgetscrpos(hillfor(x-jump,y),&xc,&yc);
          PPgetscrpos(hillfor(x,y),&xd,&yd);
          int dx=intmap.pos[x][y]-intmap.pos[x-jump][y]+intmap.pos[x][y-jump]-intmap.pos[x-jump][y-jump];
          int dy=intmap.pos[x][y]-intmap.pos[x][y-jump]+intmap.pos[x-jump][y]-intmap.pos[x-jump][y-jump];
          V3d v=V3d(-dx,-dy,jump);
          float f=V3d::normdot(v,V3d(8,8,12));
          int c=128+32+(128-32)*f;
          j.fillquad(xa,ya,xb,yb,xc,yc,xd,yd,c);
          // xa=xb; ya=yb;
        }
      }
    }
