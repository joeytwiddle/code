    ...
    for (int x=0;x<scrwid;x+=jump)
      for (int y=0;y<scrhei;y+=jump) {       
        ...
        for (int i=1;i<=xs.len;i++) {
          S s=xs.num(i);
          sum+=s.amp*sin(s.off+s.fre*(x-cenx));
        }
        for (int i=1;i<=ys.len;i++) {
          S s=ys.num(i);
          sum+=s.amp*sin(s.off+s.fre*(y-ceny));
        }
        ...
        if (x>0 && y>0) {
          ...
          for (int i=0;i<=jump;i++)
            for (int k=0;k<=jump;k++) {
              ...
              float nb=1.0-b;
              int c=quantiseto(20,na*nb*nw+a*nb*ne+na*b*sw+a*b*se);
