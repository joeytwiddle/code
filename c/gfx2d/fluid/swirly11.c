...
const gridsize=20;
const float arrowsize=30000.0;
const float browniansize=0.00005;
const traillength=1;
const float dampening=0.99;
const float holderstrength=0.06;
const float walkrdamp=0.9;
const float walktdamp=0.99;
const float walkdrdamp=0.999;
const float walkdtdamp=0.99;
const float walkdrrnd=0.00001;
const float walkdtrnd=0.005;
const float lattsize=0.5;
const float lattstrength=0.00002;
const float lattxv=0.0001;
const float lattyv=0.0005;
...
vector forceat(vector v)
{
  ...
  tmp=Vrotate(V(2.0*mymod(v.x-frame*lattxv,lattsize)/lattsize-1.0,2.0*mymod(v.y-frame*lattyv,lattsize)/lattsize-1.0),pi/2.0+pi/4.0);
  return Vmult(lattstrength*1.0/radius(tmp),Vnorm(tmp));
}
...
void movepart(part* pa)
{
  ...
  tmp=forceat(pa->pos);
  brownian(pa);
/*  pa->vel=Vadd(Vmult(dampening,pa->vel),Vmult(strength,Vadd(tmp,wibbly(pa))));*/
...
void decoration()
{
  ...
  dx=mymod(frame*lattxv,lattsize);
  dy=mymod(frame*lattyv,lattsize);
  BWClearArea(window1,0,0,SCR_WIDTH,SCR_HEIGHT);
  n=1/lattsize;
  for (i=-n*lattsize; i<=n*lattsize; i=i+lattsize) {
    plotarrow(window1,V(i+dx,1),V(0,-2));
    plotarrow(window1,V(1,i+dy),V(-2,0));
  }
}
