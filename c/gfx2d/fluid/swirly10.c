...
const gridsize=20;
const float arrowsize=1000.0;
const float browniansize=0.0005;
const traillength=1;
const float dampening=0.9;
...
const float lattstrength=0.0003;
const float lattxv=0.0000;
...
vector forceat(vector v)
{
  ...
  tmp=Vrotate(V(2.0*mymod(v.x-frame*lattxv,lattsize)/lattsize-1.0,2.0*mymod(v.y-frame*lattyv,lattsize)/lattsize-1.0),pi/2.0+pi/16.0);
...
void decoration()
{
  ...
  for (i=-n*lattsize; i<=n*lattsize; i=i+lattsize) {
    plotarrow(window1,V(i+frame*lattxv,1),V(0,-2));
    plotarrow(window1,V(1,i+frame*lattyv),V(-2,0));
  }
}
