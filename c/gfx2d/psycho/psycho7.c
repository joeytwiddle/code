// Psycho Delia
// with integer maths
// experiment - accforward instead of lookback mapping
// seems that this accforward method is slower
  ...
  while(!key[KEY_ESC]) {
    ...
    for (x=0; x<scrwid; x++) {
      for (y=0; y<scrhei; y++) {
        ...
        for (i=0;i<=1;i++) {
          for (j=0;j<=1;j++) {
            img2[piy[x][y][usingmap]+j][pix[x][y][usingmap]+i]+=amount[x][y][i][j][usingmap]*c/accuracy;
          }
        }
      }
    }
