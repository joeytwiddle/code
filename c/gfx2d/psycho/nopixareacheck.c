...
/*  amount=new (float****)[scrwid];
  for (int i=0;i<scrwid;i++) {
    amount[i]=new (float***)[scrhei];
    for (int j=0;j<scrhei;j++) {
      amount[i][j]=new (float**)[2];
      for (int k=0;k<2;k++) {
        amount[i][j][k]=new (float*)[2];
        for (int l=0;l<2;l++)
          amount[i][j][k][l]=new float[2];
      }
    }
  }
  pix=new (int**)[scrwid];
  piy=new (int**)[scrwid];
  for (int i=0;i<scrwid;i++) {
    pix[i]=new (int*)[scrhei];
    piy[i]=new (int*)[scrhei];
    for (int j=0;j<scrhei;j++) {
      pix[i][j]=new int[2];
      piy[i][j]=new int[2];
      for (int k=0;k<2;k++) {
