...
//    String c=Smid(word,intrnd(1,Slen(word)),1);
    ...
    if (myrnd()<0.07)
    font.centerString("retro",&j,myrnd()*scrhei,0.5+0.5*myrnd()*size,255);
    if (myrnd()<0.07)
    font.centerString("asymptotic",&j,myrnd()*scrhei,0.5+0.5*myrnd()*size,255);
    if (myrnd()<0.07)
    font.centerString("cyber druid",&j,myrnd()*scrhei,0.5+0.5*myrnd()*size,255);
    j.writetoscreen();
    j.fadeby(10);
    j.smooth(1);
    if (myrnd()<0.1)
