...
void main(void) {
  ...
  Map2d<float> dzdxmap=Map2d<float>(scrwid,scrhei);
  Map2d<float> dzdymap=Map2d<float>(scrwid,scrhei);  
...
//        float f=mymod(0.5+dot*0.5*scale);
        ...
        dzdxmap.pos[x][y]=dzdx;
        dzdymap.pos[x][y]=dzdy;
        ...
        if (x>0 && y>0) {
          ...
          for (int i=0;i<jump;i++)
            for (int j=0;j<jump;j++) {
              ...
              float idzdx=(1.0-a)*(1.0-d)*nwdx+(a)*(1.0-d)*nedx+(1.0-a)*(d)*swdx+(a)*(d)*sedx;
              float idzdy=(1.0-a)*(1.0-d)*nwdy+(a)*(1.0-d)*nedy+(1.0-a)*(d)*swdy+(a)*(d)*sedy;
              V3d v=(V3d(idzdx,idzdy,beddist).norm());
              int rx=x-jump+i+refract*v.x/v.z;
              int ry=y-jump+j+refract*v.y/v.z;
