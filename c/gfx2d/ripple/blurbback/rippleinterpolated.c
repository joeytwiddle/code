...
/*            if (dzdx==0 && dzdy==0)
        ...
        if (x>0 && y>0) {
          int nw=getpixel(buffer,x-res,y-res);
          int ne=getpixel(buffer,x,y-res);
          int sw=getpixel(buffer,x-res,y);
          int se=getpixel(buffer,x,y);
          for (int i=0;i<res;i++)
            for (int j=0;j<res;j++) {
              float a=(float)i/(float)res;
              float b=(float)j/(float)res;
              int d=(1.0-a)*(1.0-b)*nw+(a)*(1.0-b)*ne+(1.0-a)*(b)*sw+(a)*(b)*se;
              putpixel(buffer,x-res+i,y-res+j,d);
            }
        }
