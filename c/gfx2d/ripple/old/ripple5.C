...
void main(void) {
    ...
    float dzdx,dzdy,a,b,dot,norm,tmp,thru,amp;
    long p,q;
        ...
        for (y=0; y<scrhei; y=y+res) {
            ...
            for (i=0;i<numripples;i++) {
                ...
                if ((xy>2)&&(xy<ripples[i].front)&&(xy>ripples[i].back)) {
                    //printf("*********");
                    thru=(xy-ripples[i].back)/ripples[i].w;
                    amp=exp(-xy*3)*exp(-thru*3);
                    ...
                    //printf("n%f",norm);
                    dzdx=dzdx+amp*cos(ripples[i].n*thru*2*pi)*2*pi*ripples[i].n*(x-ripples[i].x)/xy;
                    dzdy=dzdy+amp*cos(ripples[i].n*thru*2*pi)*2*pi*ripples[i].n*(y-ripples[i].y)/xy;
