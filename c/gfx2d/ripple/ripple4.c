...
void main(void) {
  ...
  PALLETE my_pallete;
  BITMAP *scr_buffer;
  scr_buffer = load_bitmap("sm.pcx",my_pallete);
  ...
  numframes=0;
  while(!key[KEY_ESC]) {
    for (register x=0; x<scrwid; x=x+jump) {
      for (register y=0; y<scrhei; y=y+jump) {
        dzdx=0;
        dzdy=0;
        for (register i=0;i<numripples;i++) {
        ...
        for (register i=0;i<numripples;i++) {
          ...
          if ((xy>2)&&(xy<ripples[i].front)&&(xy>ripples[i].back)) {
            // printf("*********");
            a=pi/ripples[i].w*mysin(2*pi*ripples[i].n/ripples[i].w*(xy-ripples[i].v*(mytime-ripples[i].st)));
            // printf("a%f",a);
...
//        float f=mymod(0.5+dot*0.5*scale);
        ...
        if (x>0 && y>0) {
          ...
          for (register int i=0;i<jump;i++)
            for (register int j=0;j<jump;j++) {
